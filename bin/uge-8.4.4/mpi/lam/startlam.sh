...
#
# usage: startlam.sh [options] <pe_hostfile> <path to LAM-MPI>
#
#        options are:
#                     -catch_hostname
...
#
# startup of LAM MPI conforming with the Grid Engine
...
# ensure lam directory is correct
if [ ! -d $lam_dir ]; then
   echo "$me: lam installation directory $lam_dir does not exist" >&2
   exit 1
fi
if [ ! -x $lam_dir/bin/lamd_wrapper ]; then
   echo "$me: $lam_dir/bin/lamd_wrapper does not exist or is not executable" >&2
   exit 1
fi
...
#
# lamboot, make sure we use the correct LAM installation
#
PATH=$lam_dir/bin:$PATH
export PATH
lamboot -prefix $lam_dir -ssi boot rsh -ssi rsh_agent rsh $machines
