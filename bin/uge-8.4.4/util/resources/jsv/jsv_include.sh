...
#  All Rights Reserved.
#
#  Portions of this code are Copyright 2011-2012 Univa Corporation.
#
##########################################################################
#___INFO__MARK_END__
...
__jsv_cli_params="a ar A b ckpt cwd C display dl e hard h hold_jid\
                hold_jid_ad i inherit j jc js m M masterq notify\
                now N noshell nostdin o ot P p pty R r shell sync S t\
                tc tcon terse u w wd"
...
#     masterl: hard resource requests for the master task of PE jobs (-masterl ...)
...
__jsv_mod_params="ac l_hard l_soft masterl q_hard q_soft pe_min pe_max pe_name\
...
#        and "masterl" require a resource name (e.g "h_vmem"). "masterq", 
#        "q_hard", "q_soft" require a cluster queue name or a queue instance
#        name.
#
#  RESULT
#     "true" or "false"
...
###### jsv/jsv_set_timeout() ###################################################
#  NAME
#     jsv_set_timeout() -- Changes the timeout value
#
#  SYNOPSIS
#     jsv_set_timeout(timeout)
#
#  FUNCTION
#     This function can be used in jsv_on_start() to change the maximum 
#     response time of a client or server JSV. This will overwrite the value
#     specified by the environment variable SGE_JSV_TIMEOUT for client JSV's
#     Used in server JSV's it will overwrite either the default timeout value
#     of 10 seconds or the value that was specified in with jsv_timeout
#     in the qmaster_params.
#
#     The new value must be greater than 0. If the timeout has been reach, the JSV
#     will only try to re-start once, if the timeout is reached again an error 
#     will occur.
# 
#  SEE ALSO
#     jsv/jsv_on_start()
...
jsv_set_timeout()
{
   jsv_send_command "SEND TIMEOUT $1"
}
