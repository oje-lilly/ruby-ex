...
{
   if [ "$LO" = true ]; then
      $INFOTEXT -u "\nWelcome to the License Orchestrator Upgrade Procedure"
   else
...
{
   backup_dir="${1:?backup dir is required}"

   ADMIN_USER=`BootstrapGetValue "$backup_dir/cell" admin_user`
...
GetBackupDirectory()
{
   ...
   while [ $done = false ]; do
      $CLEAR
      if [ "$LO" = true ]; then
         $INFOTEXT -u "\nType the complete path to the License Orchestrator configuration backup directory."
      else
...
dbwriter.conf"
   ...
   #BINARY PATH
   ReplaceLineWithMatch "$SGE_ROOT/$SGE_CELL/common/bootstrap" 666 'binary_path.*' \
      "binary_path             $SGE_ROOT/bin"
   #QMASTER SPOOL DIR
   ReplaceLineWithMatch "$SGE_ROOT/$SGE_CELL/common/bootstrap" 666 'qmaster_spool_dir.*' \
      "qmaster_spool_dir       $QMDIR"
   #PRODUCT MODE
   ReplaceLineWithMatch "$SGE_ROOT/$SGE_CELL/common/bootstrap" 666 'security_mode.*' \
      "security_mode           $PRODUCT_MODE"
   ...
   AddLineIfNotExisting "$SGE_ROOT/$SGE_CELL/common/bootstrap" "^listener_threads " \
      "listener_threads        2"

   AddLineIfNotExisting "$SGE_ROOT/$SGE_CELL/common/bootstrap" "^worker_threads " \
      "worker_threads          4"

   AddLineIfNotExisting "$SGE_ROOT/$SGE_CELL/common/bootstrap" "^scheduler_threads " \
      "scheduler_threads       1"

   # new in UGE 8.2.0
   if [ -z "$SGE_READER_THREADS" ]; then
      reader_threads=2
   else
      reader_threads=$SGE_READER_THREADS
   fi
   AddLineIfNotExisting "$SGE_ROOT/$SGE_CELL/common/bootstrap" "^reader_threads " \
      "reader_threads          $reader_threads"

   AddLineIfNotExisting "$SGE_ROOT/$SGE_CELL/common/bootstrap" "^lo_threads " \
      "lo_threads              0"

   if [ "$SGE_ENABLE_JMX" = "true" ]; then
      ReplaceOrAddLine "$SGE_ROOT/$SGE_CELL/common/bootstrap" 666 'jvm_threads.*' \
         "jvm_threads             1"
   else
      ReplaceOrAddLine "$SGE_ROOT/$SGE_CELL/common/bootstrap" 666 'jvm_threads.*' \
         "jvm_threads             0"
...
AskForNewSequenceNumber()
{
   ...
   while [ "$done" != true ]; do
      ...
      # we can't use test here because 4294967295 is too large on HP11
      BC=`which bc`
      if [ $? -eq 0 ]; then
         res=`echo "if ($NEXT_SEQ_NUMBER>1) if ($NEXT_SEQ_NUMBER<4294967295) \"OK\"" | $BC`
         if [ "$res" != "OK" ]; then
            $INFOTEXT -n "\nInvalid sequence number %s!\n" \
                         "Number must be between 1 .. 4294967295" "$NEXT_SEQ_NUMBER"
         else
            NEXT_SEQ_NUMBER=`expr $NEXT_SEQ_NUMBER`
            done=true
         fi
      else
         $INFOTEXT -n "\nThe bc command could not be found!\nInstall the bc package for your operating system!"
      fi
      $INFOTEXT -wait -auto $AUTO -n "\nHit <RETURN> to continue >> "
   done
   $CLEAR
...
RestoreSequenceNumberFiles()
{
   ...
   QMASTER_SPOOL_DIR=$2
   AskForNewSequenceNumber "${backup_dir}/jobseqnum" "job"
   SafelyCreateFile "$QMASTER_SPOOL_DIR/jobseqnum" 644 $NEXT_SEQ_NUMBER
   AskForNewSequenceNumber "${backup_dir}/arseqnum" "AR"
...
SelectNewSpooling() 
{
   backup_copy_dir="${1:?Backup dir is required}"
   backup_spooling_method=$2

   if [ "$backup_spooling_method" != "berkeleydb" -a "$backup_spooling_method" != "classic" -a "$backup_spooling_method" != "postgres" ]; then
      $INFOTEXT "Invalid arg $backup_spooling_method to SelectNewSpooling"
      exit 1
   fi
...
sched_configuration"
      ...
      suggested_spoooling_params=`BootstrapGetValue ${backup_copy_dir}/$SGE_CELL/common "spooling_params"`
...
PrepareConfiguration()
{
   ...
   $CLEAR
   tmp_range=`FileGetValue "$backup_dir/configurations/global" "gid_range"`
   ...
   GetConfiguration "$SGE_ROOT/$SGE_CELL/spool" `FileGetValue "$backup_dir/configurations/global" "administrator_mail"`
}
