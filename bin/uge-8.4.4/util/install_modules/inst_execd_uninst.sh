...
WelcomeUninstall()
{
   ...
   if [ "$LO" = true ]; then
      $INFOTEXT -u "License Orchestrator uninstallation"
   else
...
DisableQueue()
{
   ...
   for q in `$qstat_bin -F -l h=$exechost | grep qname | cut -d"=" -f2`; do

     $INFOTEXT "Disabling queue %s now" $q
     $INFOTEXT -log "Disabling queue %s now" $q
     $qmod_bin -d $q@$exechost
...
RescheduleJobs()
{
   ...
   for q in `$qstat_bin -F -l h=$exechost | grep qname | cut -d"=" -f2`; do
     ...
     $qmod_bin -r $q@$exechost

   done

   for q in `$qstat_bin -ne -F -l h=$exechost | grep qname | cut -d"=" -f2`; do
...
RemoveQueues()
{
   ...
   for q in `$qstat_bin -F -l h=$exechost | grep qname | cut -d"=" -f2`; do
     ...
     for hgrp in `$qconf_bin -shgrpl`; do
         $SGE_BIN/$qconf_bin -dattr hostgroup hostlist $exechost $hgrp
     done
...
StopExecd()
{
   ...
   $INFOTEXT "Stopping exec daemon %s now!" $exechost
   $INFOTEXT -log "Stopping exec daemon %s now!" $exechost

   $qconf_bin -ke $exechost
   sleep 1
}
...
RemoveReferences()
{
   ...
   for q in `$qconf_bin -sql`; do
      $qconf_bin -purge queue "*" "$q@$exechost" 
   done
...
RemoveSpoolDir()
{
   ...
   SPOOL_DIR=`$qconf_bin -sconf | grep execd_spool_dir | awk '{ print $2 }'`
   ...
   SPOOL_DIR=`$qconf_bin -sconf $exechost | grep execd_spool_dir | awk '{ print $2 }'`
...
doUninstall() {
   ...
   if [ $ret = 0 ]; then
      ...
      CheckIfClusterNameAlreadyExists "execd" "uninstall"
