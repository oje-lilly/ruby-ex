...
CheckQmasterInstallation()
{
   if [ $AUTO = true ]; then
    ...
    $INFOTEXT -n "\nPlease enter cell name which you used for the %smaster\n" \
                 "installation or press <RETURN> to use [%s] >> " $MASTER_PREFIX_S $SGE_CELL
    INP=`Enter $SGE_CELL`
    if [ "$INP" = "" ]; then
       SGE_CELL=default
    else
   ...
   if [ ! -f $COMMONDIR/act_qmaster -o ! -f $COMMONDIR/bootstrap ]; then
      $INFOTEXT "\nObviously there was no %smaster installation yet!\nCall >inst_%s<\n" \
                  "on the machine which shall run the %s %master\n" $MASTER_PREFIX_S $PRODUCT_SHORT_S \
                  "$PRODUCT" $PRODUCT_SHORT_S
      $INFOTEXT -log "\nObviously there was no %smaster installation yet!\nCall >inst_%s<\n" \
                  "on the machine which shall run the %s %smaster\n" $MASTER_PREFIX_S $PRODUCT_SHORT_S \
                  "$PRODUCT" $PRODUCT_SHORT_S
...
CheckHostNameResolving()
{
   ...
   . $root_dir/util/install_modules/inst_qmaster.sh
   while [ $done = false ]; do
      $CLEAR
      ...
      errmsg=""
      $SGE_BIN/$conf_cmd -sh > /dev/null 2>&1
      if [ $? = 1 ]; then
         errmsg=`$SGE_BIN/$conf_cmd -sh 2>&1`
      else
         errmsg=`$SGE_BIN/$conf_cmd -sh 2>&1 |  grep denied:`
      fi
      
      if [ "$errmsg" != "" ]; then
         $INFOTEXT "\nCannot contact %smaster. The command failed:\n\n" \
                     "   %s\n\n" \
                     "The error message was:\n\n" \
                     "   %s\n\n" \
                     "$SGE_BIN/%s -sh" $MASTER_PREFIX_S $conf_cmd "$errmsg" $conf_cmd
         $INFOTEXT -log "\nCannot contact %smaster. The command failed:\n\n" \
                     "   %s\n\n" \
                     "The error message was:\n\n" \
                     "   %s\n\n" \
         ...
         $INFOTEXT "You can fix the problem now or abort the $mode procedure.\n" \
                   "The problem can be:\n\n" \
                   "   - the %smaster is not running\n" \
                   "   - the %smaster host is down\n" \
                   "   - an active firewall blocks your request\n" $MASTER_PREFIX_S $MASTER_PREFIX_S
         $INFOTEXT -auto $AUTO -ask "y" "n" -def "y" -n "Contact %smaster again (y/n) ('n' will abort) [y] >> " $MASTER_PREFIX_S
         if [ $? != 0 ]; then
            $INFOTEXT "$MODE failed"
            $INFOTEXT -log "Cannot contact %smaster! $MODE failed" $MASTER_PREFIX_S

            MoveLog
            exit 1
         fi
      else
         ignore_fqdn=`cat $root_dir/$cell_dir/common/bootstrap | grep "^ignore_fqdn" | awk '{print $2}'| egrep -i "true|1"`
         ...
         default_domain=`cat $root_dir/$cell_dir/common/bootstrap | grep "^default_domain" | awk '{print $2}' | tr "[A-Z]" "[a-z]"`
         ...
         if [ $ignore_fqdn = true ]; then
            admin_host_list=`ExecuteAsAdmin $SGE_BIN/$conf_cmd -sh | cut -f1 -d. | tr "[A-Z]" "[a-z]"`
            myname=`echo $myaname | cut -f1 -d. | tr "[A-Z]" "[a-z]"`
         else
            admin_host_list=`ExecuteAsAdmin $SGE_BIN/$conf_cmd -sh | tr "[A-Z]" "[a-z]"`
         ...
         if [ $found = false ]; then
            $INFOTEXT "\nThis hostname is not known at %smaster as an administrative host.\n\n" \
                        ...
                        $MASTER_PREFIX_S $myrealname $myaname $default_domain $ignore_fqdn $myname

            if [ $AUTO = true ]; then
               $INFOTEXT -log "$MODE failed!\nThis hostname is not known at %smaster as an administrative host." $MASTER_PREFIX_S
               MoveLog
               exit 1
            fi
           
            $INFOTEXT "Please check and correct your >/etc/hosts< file and >/etc/nsswitch.conf<\n" \
                      "file on this host and on the %smaster machine.\n\n" \
            ...
            $INFOTEXT -wait -auto $AUTO -n "\nThis hostname is known at %smaster as an administrative host.\n\n" \
                            "Hit <RETURN> to continue >>" $MASTER_PREFIX_S
            $CLEAR
            done=true
         fi
      fi
...
AddLocalConfiguration_With_Qconf()
{
   ...
   if [ -f $TMPL ]; then
      ...
      $SGE_BIN/$CONF_CMD -sconf $HOST > $TMPL 2>/dev/null
      if [ $? -eq 0 ]; then
         ...
         ExecuteAsAdmin $SGE_BIN/$CONF_CMD -Mconf $TMPL
      else
         PrintLocalConf 0 "$1" > $TMPL
         ExecuteAsAdmin $SGE_BIN/$CONF_CMD -Aconf $TMPL
      fi
      rm -rf /tmp/$$
...
StartExecd()
{
   $INFOTEXT -u "\n%s execution daemon startup" "$PRODUCT"
   $INFOTEXT "\nStarting execution daemon. Please wait ..."
   if [ "$SGE_ENABLE_SMF" = "true" ]; then
      if [ -z "$SGE_CLUSTER_NAME" -a "$LO" = false ]; then
         SGE_CLUSTER_NAME=`cat $SGE_ROOT/$SGE_CELL/common/cluster_name 2>/dev/null`
      elif [ -z "$LO_CLUSTER_NAME" -a "$LO" = true ]; then
         SGE_CLUSTER_NAME=`cat $LO_ROOT/$LO_CELL/common/cluster_name 2>/dev/null`
      fi
      ...
      if [ "$RC_FILE" = "systemctl" -a "$USE_SYSTEMD" = "true" -a "$SYSTEMD_ADD2RC" = "0" ]; then
         $SYSTEMCTL start sgeexecd.${SGE_CLUSTER_NAME}.service
      else
         $SGE_STARTUP_FILE
      fi 
   fi
...
AddQueue()
{
   ...
   # on Windows 7, slots is now "1 \r", so trim it
   slots=`echo $slots | tr '\n' ' '`
   slots=`echo $slots | tr '\r' ' '`

   # remove spaces by just doing echo again
   slots=`echo $slots`
   ...
   if [ $? = 0 ]; then
      $SGE_BIN/$CONF_CMD -aattr hostgroup hostlist $exechost @allhosts
      if [ "$LO" = true ]; then
         $SGE_BIN/$CONF_CMD -aattr queue slots "[$exechost=9999999]" all.q
      else
         $SGE_BIN/$CONF_CMD -aattr queue slots "[$exechost=$slots]" all.q
      fi
      $INFOTEXT -wait -auto $AUTO -n "\nHit <RETURN> to continue >> "
      $CLEAR
   fi
...
GetLocalExecdSpoolDir()
{
   if [ $AUTO != "true" ]; then
      ...
      first_loop=1
      while [ "$ret" = "0" ]; do
         # first ask the question, then do the enter loop
         ...
         if [ "$LOCAL_EXECD_SPOOL" = "" ]; then
            ...
            if [ "$SGE_ARCH" = "win32-x86" ]; then
               # on Windows, the execd spool dir must be on a local disk
               IsLocalDir $GLOBAL_EXECD_SPOOL
               ask_question=$?
            else
               ask_question=1
            fi
            ...
            if [ "$ask_question" = "1" ]; then
               $INFOTEXT -n -auto $AUTO -ask "y" "n" -def "n" \
                         "Do you want to configure a different spool directory\n" \
                         "for this host (y/n) [n] >> "
               ret=$?
            else
               # we get here only on Windows if the GLOBAL_EXECD_SPOOL dir is not local
               $INFOTEXT "The current spool directory is not located on a local disk\n" \
      ...
      if [ -n "$EXECD_SPOOL_DIR_LOCAL" ]; then
         spooldir_valid=1
...
CheckWinLocalAdministrator()
{
   if [ "$SGE_ARCH" != "win32-x86" ]; then
      return
   fi

   # The effective user ID of the current user must be 197108 (=local Administrator)
   # if this is a root user or SGE admin user installation.
   # It must be the ID of the SGE admin user if this is a test installation.
   if [ -f $SGE_ROOT/$SGE_CELL/common/bootstrap ]; then
      sge_admin_user=`cat $SGE_ROOT/$SGE_CELL/common/bootstrap | grep admin_user | awk '{ print $2 }'`
      if [ "$sge_admin_user" = "root" -o "$sge_admin_user" = "none" ]; then
         # root user system - the current user must be the local Administrator
         SYSTEM="root"
      else
         # SGE admin user system or test user system
         # If the current user name is the name of the SGE admin user, it's a test user system
         # Get the name of the current user without any domain specifier
         current_user=`id -u -n | cut -d"+" -f2`
         if [ "$current_user" = "$sge_admin_user" ]; then
            # test user system - let the check always succeed
            SYSTEM="test"
         else
            # SGE admin user system - the current user must be the local Administrator
            SYSTEM="admin"
         fi
      fi
   else
      $INFOTEXT "bootstrap file could not be found, please check your installation! Exiting now ..."
      $INFOTEXT -log "bootstrap file could not be found, please check your installation! Exiting now ..."
      exit 1
   fi

   current_uid=`id -u`
   if [ "$SYSTEM" = "root" -o "$SYSTEM" = "admin" ]; then
      if [ "$current_uid" != "197108" ]; then
         $INFOTEXT "It seems the Master host was installed by root, but the current user\n" \
                   "is not the local Administrator. Therefore installing is not possible!\n\n" \
                   "On Interix (the POSIX subsystem of Microsoft Windows), only the local\n" \
                   "Administrator is a superuser like root on Unix/Linux. Neither any other\n" \
                   "member of any Administrators group nor the Domain Administrator have\n" \
                   "the necessary, root-like permissions.\n\n" \
                   "The local Administrator on this host is \"%s\", \n" \
                   "his user ID is 197108.\n\n" \
                   "Please switch to the local Administrator user and run the installation again!\n\n" \
                   "If this is a test installation and the Master host was installed by a normal\n" \
                   "user, then please switch to this user and run the installation again!\n\n" \
                   `id -u -n 197108`

         $INFOTEXT -log "It seems the Master host was installed by root, but the current user\n" \
                   "is not the local Administrator. Therefore installing is not possible!\n\n" \
                   "On Interix (the POSIX subsystem of Microsoft Windows), only the local\n" \
                   "Administrator is a superuser like root on Unix/Linux. Neither any other\n" \
                   "member of any Administrators group nor the Domain Administrator have\n" \
...
CheckWinAdminUser()
{
   ...
   if [ "$win_admin_user" != "default" -a "$win_admin_user" != "root" -a "$win_admin_user" != "none" ]; then
      WIN_HOST_NAME=`hostname -s | tr "[a-z]" "[A-Z]"`
      ...
      if [ "$ret" != 0 ]; then
         while [ "$ret" != 0 ]; do
            ...
            while [ "$SECRET1" != "$SECRET2" ]; do
               ...
               $INFOTEXT "Please confirm the password now >> "
               stty_orig=`stty -g`
               stty -echo
               read SECRET2
               stty $stty_orig
               ...
               if [ "$SECRET1" != "$SECRET2" ]; then
                  $INFOTEXT "The passwords do not match, please try again."
               fi
            done

            $INFOTEXT "Creating admin user %s, now ...\n" $win_admin_user
            eval net user $win_admin_user $SECRET1 /add < /dev/null
            ret=$?
            unset SECRET1
            unset SECRET2
         done
         ExecuteAsAdmin regpwd
         $INFOTEXT -wait "Admin user created, hit <RETURN> to continue!"
         MAPADMIN_EXE=`which mapadmin.exe` 2> /dev/null
         if [ -x "$MAPADMIN_EXE" ]; then
            ...
            $INFOTEXT "Please configure the necessary NFS mappings for the new\n " \
                      "user $WIN_HOST_NAME"+"$win_admin_user to the Unix user $win_admin_user\n" \
                      "on the NFS name mapping server."
            $INFOTEXT -wait "Hit <RETURN> to continue!"
         fi
      fi
      PATH=$tmp_path
      export PATH 
...
InstallWinSvc()
{
   ...
   while [ "$ret" -ne "0" -a "$loop" -lt 6 ]; do
      eval "$WIN_DIR/$WIN_SVC -install $admin_name $admin_password" < /dev/null > /dev/null 2>&1
      ret=$?
      loop=`expr $loop + 1`
      sleep 2
   done
...
CopyWinSvcBinary()
{
   ...
   $INFOTEXT -log "\n   ... moving new service binary!"
   cp -fR $SGE_UTILBIN/$WIN_SVC $WIN_DIR
}
...
InstWinHelperSvc()
{
   ...
   UninstallWinSvc "$WIN_SVC" "$WIN_DIR"
   CopyWinSvcBinary "$WIN_SVC" "$WIN_DIR"
   InstallWinSvc "$WIN_SVC" "$WIN_DIR"
   StartWinSvc "$WIN_SVC"

   # Additionally copy the SGE_Starter.exe, which is not a service but is
   # needed by the SGE_Helper_Service.exe
