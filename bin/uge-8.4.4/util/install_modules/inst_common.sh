...
#  All Rights Reserved.
#
#  Portions of this software are Copyright (c) 2011-2014 Univa Corporation
#
##########################################################################
#___INFO__MARK_END__
...
CheckCmdParamsLO()
{
   ...
   while [ $ARGC != 0 ]; do
      ...
      -rst)
        ...
        ;;
      -upd)
        #update from 6.2u5 or higher to 8.1.0
        UPDATE=true
        QMASTER="install"
        POST_UPDATE=true
        . ./util/install_modules/inst_qmaster.sh
      ...
      -rccreate)
        ...
        ;;
      -rc_no_systemd)
        USE_SYSTEMD=false; export USE_SYSTEMD
        # disable the systemd startup scripts even when systemd is available
        ;;
      -noremote)
        NOREMOTE=true
...
sol*|lx*)
  ;;
  lx*)
     # do nothing, the rpath is set to the libraries
     ;;
  *)
  ...
  if [ "$LO" = true ]; then
     SGE_MASTER_NAME=lo_master
     SGE_EXECD_NAME=lo_execd
     SGE_SHEPHERD_NAME=lo_shepherd
     SGE_SHADOWD_NAME=lo_shadowd
     SGE_SERVICE=lo_master
     SGE_QMASTER_SRV=lo_master
     SGE_EXECD_SRV=lo_execd
  else
...
ExecuteAsAdmin()
{
   ...
   if [ "$ADMINUSER" = default -o "$SGE_ARCH" = "win32-x86" ]; then
      "$@"
   else
   ...
   if [ $? != 0 ]; then
      ...
      if [ "$insideMoveLog" != true ]; then
         #To prevent endless recursion when failure occures in the MoveLog due to perm problems when moving the log file
         MoveLog
      fi
...
CheckBinaries()
{
   ...
   if [ "$LO" = true ]; then
      BINFILES="lo_execd lo_master lo_shadowd lo_shepherd loacct loconf lodel \
                lostat losub loquota lomod"
   else
      ...
      if [ "$SGE_ARCH" != "aix-ppc64" ]; then
         BINFILES="$BINFILES qmon"
      fi
      UTILFILES="qrsh_starter"
   fi
   ...
   UTILFILES="$UTILFILES adminrun checkprog checkuser filestat gethostbyaddr gethostbyname \
              gethostname getservbyname loadcheck now rlogin rsh rshd \
   ...
   if [ $missing = true ]; then
      if [ "$LO" = true ]; then
         $INFOTEXT "\nMissing License Orchestrator binaries!\n\n" \
         "A complete installation needs the following binaries in >%s<:\n\n" \
         "loacct          lo_shepherd\n" \
         "loconf          lo_execd\n" \
         "lodel           lostat           lo_master\n" \
         "qresub          losub            loquota lomod\n" \
         "lo_shadowd\n" \
...
#-------------------------------------------------------------------------
# ErrUsageLO: print usage string, exit
#
ErrUsageLO()
{
   myname=`basename $0`
   $INFOTEXT -e \
             "Usage: %s -lo|-ulo|-sm|-usm|-db|-udb|-bup|-rst| \n" \
             "       -copycerts <host|hostlist>|-v|-upd|-upd-rc| \n" \
             "       -post_upd|-rccreate|[-host <hostname>] [-resport] [-rsh] \n" \
             "       [-auto <filename>] [-nr] [-csp] \n" \
             "       [-afs] [-noremote] [-nosmf] [-nost]\n" \
             "   -lo              install the License Orchestrator\n" \
             "   -ulo             uninstall the License Orchestrator\n" \
             ...
             "   -v               print version\n" \
             "   -upd             upgrade the License Orchestrator\n" \
             "   -upd-rc          create new autostart scripts for the LO\n" \
             "   -post-upd        finish the upgrade procedure\n" \
             "   -rccreate        create startup scripts from templates\n" \
             "   -host            hostname for shadow master installation or uninstallation \n" \
             "                    (eg. exec host)\n" \
             "   -resport         the install script does not allow LO_MASTER_PORT numbers \n" \
             "                    higher than 1024\n" \
             "   -rsh             use rsh instead of ssh (default is ssh)\n" \
             "   -auto            full automatic installation\n" \
             ...
             "   -verify          verify the version of installed binaries\n" \
             "   -help            show this help text\n\n" \
             "   Examples:\n" \
             "   inst_lo -lo   or  inst_lo\n" \
             "                     Installs the License Orchestrator\n" \
             "   inst_lo -lo -auto /path/to/config-file\n" \
             "                     Installs the License Orchestrator using the given\n" \
             "                     configuration file\n" \
             "                     (A template can be found in:\n" \
             "                     util/install_modules/inst_template.conf)\n" \
             "   inst_lo  -db      Install a Berkeley DB Server on local host\n" \
             "   inst_lo  -sm      Install a Shadow Master Host on local host\n" \
             "   inst_lo -copycerts host or inst_lo -copycerts \"host1 host2\"\n" $myname
...
ErrUsage()
{
   ...
   $INFOTEXT -e \
             ...
             "   -rccreate  create startup scripts from templates\n" \
             "   -rc_no_systemd   disable the use of Systemd startup scripts\n" \
             "   -host      hostname for shadow master installation or uninstallation \n" \
             "              (eg. exec host)\n" \
             ...
             "   -nosmf     disable SMF for Solaris 10+ machines (RC scripts are used)\n" \
             "   -nost      do not use Sun Service Tags\n" \
             "   -verify    verify the version of installed binaries on execution hosts\n" \
             "   -help      show this help text\n\n" \
             "   Examples:\n" \
...
{
   CONFIG_FILE=$1
   # the list of config file entires we know, i.e. all config file entries not
   # listed here will be rejected
   KNOWN_CONFIG_FILE_ENTRIES_INSTALL="CELL_NAME ADMIN_USER ADMIN_PASSWORD QMASTER_SPOOL_DIR EXECD_SPOOL_DIR GID_RANGE SPOOLING_METHOD DB_SPOOLING_SERVER DB_SPOOLING_DIR PG_SPOOLING_ARGS PAR_EXECD_INST_COUNT ADMIN_HOST_LIST SUBMIT_HOST_LIST EXEC_HOST_LIST EXECD_SPOOL_DIR_LOCAL HOSTNAME_RESOLVING SHELL_NAME COPY_COMMAND DEFAULT_DOMAIN ADMIN_MAIL ADD_TO_RC SET_FILE_PERMS RESCHEDULE_JOBS SCHEDD_CONF SHADOW_HOST EXEC_HOST_LIST_RM REMOVE_RC WINDOWS_SUPPORT WIN_ADMIN_NAME WIN_ADMIN_PASSWORD WIN_DOMAIN_ACCESS CSP_RECREATE CSP_COPY_CERTS CSP_COUNTRY_CODE CSP_STATE CSP_LOCATION CSP_ORGA CSP_ORGA_UNIT CSP_MAIL_ADDRESS SGE_READER_THREADS USE_SYSTEMD"
   KNOWN_CONFIG_FILE_ENTRIES_BACKUP="BACKUP_DIR TAR BACKUP_FILE"
   # the list of deprecated config file entries, i.e. we still accept (and ignore)
   # them but print a warning message
   DEPRECATED_CONFIG_FILE_ENTRIES_INSTALL="WIN_DOMAIN_ACCESS" 

   if [ "$LO" = true ]; then
      KNOWN_CONFIG_FILE_ENTRIES_INSTALL="$KNOWN_CONFIG_FILE_ENTRIES_INSTALL LO_ROOT LO_MASTER_PORT LO_EXECD_PORT LO_ENABLE_SMF LO_CLUSTER_NAME LO_ENABLE_JMX LO_JMX_PORT LO_JVM_LIB_PATH LO_ADDITIONAL_JVM_ARGS LO_JMX_SSL LO_JMX_SSL_CLIENT LO_JMX_SSL_KEYSTORE LO_JMX_SSL_KEYSTORE_PW" 
      KNOWN_CONFIG_FILE_ENTRIES_BACKUP="LO_ROOT LO_CELL $KNOWN_CONFIG_FILE_ENTRIES_BACKUP"
   else
      KNOWN_CONFIG_FILE_ENTRIES_INSTALL="$KNOWN_CONFIG_FILE_ENTRIES_INSTALL SGE_ROOT SGE_QMASTER_PORT SGE_EXECD_PORT SGE_ENABLE_SMF SGE_CLUSTER_NAME SGE_ENABLE_JMX SGE_JMX_PORT SGE_JVM_LIB_PATH SGE_ADDITIONAL_JVM_ARGS SGE_JMX_SSL SGE_JMX_SSL_CLIENT SGE_JMX_SSL_KEYSTORE SGE_JMX_SSL_KEYSTORE_PW WIN_SGE_ROOT WIN_EXECD_SPOOL_DIR_LOCAL UNIX_SGECA_DIR WIN_SGECA_DIR UNIX_HOME_DIR WIN_HOME_DIR UNIX_TMP_DIR WIN_TMP_DIR"
      KNOWN_CONFIG_FILE_ENTRIES_BACKUP="SGE_ROOT SGE_CELL $KNOWN_CONFIG_FILE_ENTRIES_BACKUP"
   fi
   ...
   if [ "$BACKUP" = "true" ]; then
      ...
      if [ "$LO" = true ]; then
         if [ -z "$LO_ROOT" ]; then
            $INFOTEXT -e "Your >LO_ROOT< entry is not set!"
            $INFOTEXT -log "Your >LO_ROOT< entry is not set!"
            is_valid="false" 
         elif [ ! -d "$LO_ROOT" ]; then
            $INFOTEXT -e "Your >LO_ROOT< directory %s does not exist!" $LO_ROOT
            $INFOTEXT -log "Your >LO_ROOT< directory %s does not exist!" $LO_ROOT
            is_valid="false" 
         fi
         if [ -z "$LO_CELL" ]; then
            $INFOTEXT -e "Your >LO_CELL< entry is not set!" 
            $INFOTEXT -log "Your >LO_CELL< entry is not set!"
            is_valid="false"
         elif [ ! -d "$LO_ROOT/$LO_CELL" ]; then
            $INFOTEXT -e "Your >LO_CELL< directory %s does not exist!" $LO_ROOT/$LO_CELL
            $INFOTEXT -log "Your >LO_CELL< directory %s does not exist!" $LO_ROOT/$LO_CELL
            is_valid="false"
         fi
      else
         if [ -z "$SGE_ROOT" ]; then
   ...
   if [ "$QMASTER" = "install" -o "$EXECD" = "install" -o "$QMASTER" = "uninstall" -o "$EXECD" = "uninstall" ]; then
      for e in $CONFIG_ENTRIES; do
         ...
         echo $DEPRECATED_CONFIG_FILE_ENTRIES_INSTALL | grep $e > /dev/null 2>&1
         if [ $? = 0 ]; then
            $INFOTEXT -e "Please be aware that your configuration entry >%s< is deprecated, will" $e
            $INFOTEXT -e "be ignored and might be removed in future versions!" 
            $INFOTEXT -log "Please be aware that your configuration entry >%s< is deprecated, will" $e
            $INFOTEXT -log "be ignored and might be removed in future versions!" 
         fi
      done
      if [ "$LO" = true ]; then
         LO_CELL=$CELL_NAME
   ...
   if [ "$QMASTER" = "install" ]; then
      ...
      if [ "$LO" = true ]; then
         if [ -d "$LO_ROOT/$LO_CELL" -a \( -z "$DB_SPOOLING_SERVER" -o "$DB_SPOOLING_SERVER" = "none" \) ]; then
            $INFOTEXT -e "Your >CELL_NAME< directory %s already exist!" $LO_ROOT/$LO_CELL
            $INFOTEXT -e "The automatic installation stops, if the >LO_CELL< directory already exists"
            $INFOTEXT -e "to ensure, that existing installations are not overwritten!"
            $INFOTEXT -log "Your >CELL_NAME< directory %s already exist!" $LO_ROOT/$LO_CELL
            $INFOTEXT -log "The automatic installation stops, if the >LO_CELL< directory already exists"
            $INFOTEXT -log "to ensure, that existing installations are not overwritten!"
            is_valid="false"
         fi
         `IsNumeric "$LO_QMASTER_PORT"`
         if [ "$?" -eq 1 ]; then
            $INFOTEXT -e "Your >LO_QMASTER_PORT< entry is invalid. It must be empty for using a service\nor a valid port number."
            $INFOTEXT -log "Your >LO_QMASTER_PORT< entry is invalid. It must be empty for using a service\nor a valid port number."
            is_valid="false"
         elif [ "$LO_QMASTER_PORT" -le 1 -o "$LO_QMASTER_PORT" -ge 65536 ]; then
            $INFOTEXT -e "Your >LO_QMASTER_PORT< entry is invalid. It must be a number between 2 and 65535!"
            $INFOTEXT -log "Your >LO_QMASTER_PORT< entry is invalid. It must be a number between 2 and 65535!"
            is_valid="false"
         fi
         `IsNumeric "$LO_EXECD_PORT"`
         if [ "$?" -eq 1 ]; then
            $INFOTEXT -e "Your >LO_EXECD_PORT< entry is invalid. It must be empty for using a service\nor a valid port number."
            $INFOTEXT -log "Your >LO_EXECD_PORT< entry is invalid. It must be empty for using a service\nor a valid port number."
            is_valid="false"
         elif [ "$LO_EXECD_PORT" -le 1 -o "$LO_EXECD_PORT" -ge 65536 ]; then
            $INFOTEXT -e "Your >LO_EXECD_PORT< entry is invalid. It must be a number between 2 and 65535!"
            $INFOTEXT -log "Your >LO_EXECD_PORT< entry is invalid. It must be a number between 2 and 65535!"
            is_valid="false"
         fi
         `IsValidClusterName "$LO_CLUSTER_NAME"`
         if [ "$?" -eq 1 ]; then
            $INFOTEXT -e "Your >LO_CLUSTER_NAME< entry is invalid. Valid Clustername is e.g. p_1234"
            $INFOTEXT -log "Your >LO_CLUSTER_NAME< entry is invalid. Valid Clustername is e.g. p_1234"
            is_valid="false"
         fi
      else
      ...
      if [ "$SPOOLING_METHOD" != "berkeleydb" -a "$SPOOLING_METHOD" != "classic" -a "$SPOOLING_METHOD" != "postgres" ]; then
      ...
      if [ "$SPOOLING_METHOD" = "postgres" -a -z "$PG_SPOOLING_ARGS" ]; then
         $INFOTEXT -e "Your >PG_SPOOLING_ARGS< is empty. You have to enter a PostgreSQL connection string!"
         $INFOTEXT -log "Your >PG_SPOOLING_ARGS< is empty. You have to enter a PostgreSQL connection string!"
         is_valid="false"
      fi
   ...
   if [ "$QMASTER" = "install" -o "$EXECD" = "install" ]; then
      ...
      if [ "$LO" = true ]; then
         if [ -z "$LO_ENABLE_SMF" ]; then
            $INFOTEXT -e "Your >LO_ENABLE_SMF< flag is not set!"
            $INFOTEXT -log "Your >LO_ENABLE_SMF< flag is not set!"
            is_valid="false" 
         fi 
         if [ "$LO_ENABLE_SMF" = "1" ]; then
            LO_ENABLE_SMF="true"
         elif [ "$LO_ENABLE_SMF" = "0" ]; then
            LO_ENABLE_SMF="false"
         fi
         LO_ENABLE_SMF=`echo $LO_ENABLE_SMF | tr "[A-Z]" "[a-z]"`
         if [ "$LO_ENABLE_SMF" != "true" -a "$LO_ENABLE_SMF" != "false" ]; then
            $INFOTEXT -e "Your >LO_ENABLE_SMF< flag is wrong! Valid values are: 0, 1, true, false"
            $INFOTEXT -log "Your >LO_ENABLE_SMF< flag is wrong! Valid values are: 0, 1, true, false"
            is_valid="false" 
         elif [ "$LO_ENABLE_SMF" = "false" ]; then
            SMF_FLAGS="-nosmf"
         fi
      else
...
WelcomeTheUser()
{
   ...
   $INFOTEXT -u "\nWelcome to the %s installation" "$PRODUCT"
   $INFOTEXT -u "\n%s master host installation" "$PRODUCT"
   $INFOTEXT "\nBefore you continue with the installation please read these hints:\n\n" \
             ...
             "The master installation procedure will take approximately 5-10 minutes.\n"
...
CheckWhoInstallsSGE()
{
   ...
   if [ $this_dir_user != root ]; then
      ...
      $INFOTEXT "\nThe current directory\n\n" \
                ...
                "of the machines where %s will be installed (NFS partitions not\n" \
                "exported for user >root< with read/write permissions) it is recommended to\n" \
                "install %s that all spool files will be created under the user id\n" \
                "of user >%s<.\n\n" \
                "IMPORTANT NOTE: The daemons still have to be started by user >root<.\n" \
                `pwd` $this_dir_user "$PRODUCT" "$PRODUCT" $this_dir_user

      $INFOTEXT -auto $AUTO -ask "y" "n" -def "y" -n \
                "Do you want to install %s as admin user >%s< (y/n) [y] >> " "$PRODUCT" $this_dir_user
                ret=$?
      ...
      if [ "$ret" = 0 ]; then
         $INFOTEXT "Installing %s as admin user >%s<" "$PRODUCT" "$this_dir_user"
         $INFOTEXT -log "Installing %s as admin user >%s<" "$PRODUCT" "$this_dir_user"
   ...
   $INFOTEXT -u "\nChoosing %s admin user account" "$PRODUCT"
   ...
   $INFOTEXT "\nYou may install %s that all files are created with the user id of an\n" \
             ...
             "   - %s still has to be started by user >root<\n\n" \
             "   - this directory should be owned by the %s administrator\n" \
             "$PRODUCT" "$PRODUCT" "$PRODUCT" "$PRODUCT"

   $INFOTEXT -auto $AUTO -ask "y" "n" -def "y" -n \
             "Do you want to install %s\n" \
             "under an user ID other than >root< (y/n) [y] >> " "$PRODUCT"

   if [ $? = 0 ]; then
      done=false
      while [ $done = false ]; do
         $CLEAR
         $INFOTEXT -u "\nChoosing a %s admin user name" "$PRODUCT"
         ...
         if [ "$INP" != "" ]; then
            ...
            if [ $? != 0 ]; then
               ...
               $INFOTEXT "\nInstalling %s as admin user >%s<\n" "$PRODUCT" $INP
               $INFOTEXT -log "Installing %s as user >%s<" "$PRODUCT" $INP
               ADMINUSER=$INP
      ...
      $INFOTEXT "\nInstalling %s as user >root<\n" "$PRODUCT"
      $INFOTEXT -log "Installing %s as user >root<" "$PRODUCT"
      ADMINUSER=default
      $INFOTEXT -wait -auto $AUTO -n "Hit <RETURN> to continue >> "
      $CLEAR
   fi
...
# libnuma is required for lx-amd64 execution daemon installation

CheckForLIBNUMA()
{
   # This check makes sense on lx-amd64 only. On all other architectures,
   # it always succeeds.
   if [ "$SGE_ARCH" != "lx-amd64" ]; then
      return 0
   fi

   output=`$SGE_UTILBIN/loadcheck -cb`

   # search for Your Linux kernel ...
   ok=false
   ...
   if [ $ok = false ]; then
      $INFOTEXT -u "\nNUMA system library not found"
      $INFOTEXT -wait -auto $AUTO -n \
                "\nOn architecture \"lx-amd64\", the Grid Engine execution daemon installation requires the\n" \
                "NUMA system library \"libnuma.so\" to be installed. Please install the \"libnuma\" or the\n" \
                "numactl package on host \"%s\" and check if \n" \
                "\"\$SGE_ROOT/utilbin/lx-amd64/loadcheck -cb\"\n" \
                "prints meaningful data and no error messages then. Then start the installation again.\n\n" \
                "Press <RETURN> to exit the installation procedure >> " $HOST
      $INFOTEXT -log -wait -auto $AUTO -n \
                "\nOn architecture \"lx-amd64\", the Grid Engine execution daemon installation requires the\n" \
                "NUMA system library \"libnuma.so\" to be installed. Please install the \"libnuma\" or the\n" \
                "numactl package on host %s and check if \"\$SGE_ROOT/utilbin/lx-amd64/loadcheck -cb\"\n" \
                "prints meaningful data and no error messages then. Then start the installation again.\n\n" \
                "Press <RETURN> to exit the installation procedure >> " $HOST
      exit 2
   fi
...
CheckForLocalHostResolving()
{
   ...
   if [ $notok = true ]; then
      ...
      $INFOTEXT -wait -auto $AUTO -n \
                "\nIt is not supported for a %s installation that the local hostname\n" \
                ...
                "Installation failed.\n\n" \
                "Press <RETURN> to exit the installation procedure >> " "$PRODUCT"
      $INFOTEXT -log "\nThe current hostname is resolved as follows:\n\n"
      $SGE_UTILBIN/gethostname
      $INFOTEXT -log -wait -auto $AUTO -n \
                "\nIt is not supported for a %s installation that the local hostname\n" \
...
ProcessSGERoot()
{
   ...
   while [ $check_done = false ]; do
      if [ "$ROOT_DIR" = "" ]; then
         while [ "$ROOT_DIR" = "" ]; do
            $CLEAR
            $INFOTEXT -u "\nChecking \$%s_ROOT directory" $PRODUCT_SHORT
            $ECHO
            eval ROOT_DIR=`pwd | sed 's/\/tmp_mnt//'`
            $INFOTEXT -n "The %s root directory is not set!\n" \
                         "Please enter a correct path for %s_ROOT.\n" "$PRODUCT" $PRODUCT_SHORT
            $INFOTEXT -n "If this directory is not correct (e.g. it may contain an automounter\n" \
                         "prefix) enter the correct path to this directory or hit <RETURN>\n" \
                         "to use default [%s] >> " $ROOT_DIR

            eval ROOT_DIR=`Enter $ROOT_DIR`
         done
         if [ "$LO" = true ]; then
            LO_ROOT=$ROOT_DIR
            export LO_ROOT
         ...
         $ECHO
         if [ "$LO" = true ]; then
            SGE_ROOT_VAL=`eval echo $LO_ROOT`
         else
            SGE_ROOT_VAL=`eval echo $SGE_ROOT`
         fi

         $INFOTEXT -n "The %s root directory is:\n\n" \
                      "   \$%s_ROOT = %s\n\n" \
                      "If this directory is not correct (e.g. it may contain an automounter\n" \
                      "prefix) enter the correct path to this directory or hit <RETURN>\n" \
                      "to use default [%s] >> " "$PRODUCT" $PRODUCT_SHORT $SGE_ROOT_VAL $SGE_ROOT_VAL

         eval ROOT_DIR=`Enter $SGE_ROOT_VAL`
      ...
      if [ "$AUTO" = "true" -a "$check_done" = "false" ]; then
         ...
         RestoreStdout
         $INFOTEXT "Your selected \$%s_ROOT directory: %s\n is not readable/writeable for user: %s" $PRODUCT_SHORT $SGE_ROOT_VAL $output_username
         exit 2
      fi
   done

   CheckPath
   $INFOTEXT "Your \$%s_ROOT directory: %s\n" $PRODUCT_SHORT $SGE_ROOT_VAL
   $INFOTEXT -log "Your \$%s_ROOT directory: %s" $PRODUCT_SHORT $SGE_ROOT_VAL
   $INFOTEXT -wait -auto $AUTO -n "Hit <RETURN> to continue >> "
   $CLEAR
}

#-------------------------------------------------------------------------
# CheckSGERootPermissions: Try to create a file in SGE_ROOT
# as admin user or root (depending on ADMINUSER)
#
CheckSGERootPermissions() {
   ret_code=1
   ...
   if [ $ret -ne 0 ]; then
      $CLEAR
      $INFOTEXT "Can't create a temporary file in the %s_ROOT directory\n\n   %s\n\n" \
      ...
      $INFOTEXT "Your \$%s_ROOT environment variable\n\n   \$%s_ROOT = %s\n\n" \
                  "doesn't match the current directory.\n" $PRODUCT_SHORT $PRODUCT_SHORT $SGE_ROOT_VAL
...
GetDefaultClusterName() {
   if [ "$LO" = true ]; then
      if [ -z "$LO_CLUSTER_NAME" ]; then
         SGE_CLUSTER_NAME="p$LO_MASTER_PORT"
      else
         SGE_CLUSTER_NAME=$LO_CLUSTER_NAME
      fi
   else
...
# $1 ... hosttype (master|shadow|bdb|execd|dbwriter|ugerest)
#
SetupRcScriptNames61()
{
   ...
   if [ "$RC_FILE" = "systemctl" -a "$USE_SYSTEMD" = "true" ]; then
      if [ $hosttype = "master" ]; then
         script_name=sgemaster
         TMP_SGE_STARTUP_FILE="util/rctemplates/sgemaster_systemd_template"
         STARTUP_FILE_NAME=sgemaster.${SGE_CLUSTER_NAME}.service
         S95NAME=S95sgemaster.$SGE_CLUSTER_NAME
         K03NAME=K03sgemaster.$SGE_CLUSTER_NAME
         DAEMON_NAME="qmaster"
      elif [ $hosttype = "lomaster" ]; then
         script_name=lomaster
         TMP_SGE_STARTUP_FILE=/tmp/lomaster.$$
         STARTUP_FILE_NAME=lomaster.$LO_CLUSTER_NAME
         S95NAME=S95lomaster.$LO_CLUSTER_NAME
         K03NAME=K03lomaster.$LO_CLUSTER_NAME
         DAEMON_NAME="lomaster"
         ...
         TMP_SGE_STARTUP_FILE="util/rctemplates/sgeexecd_systemd_template"
         STARTUP_FILE_NAME=sgeexecd.${SGE_CLUSTER_NAME}.service
         S95NAME=S96sgeexecd.$SGE_CLUSTER_NAME
         K03NAME=K02sgeexecd.$SGE_CLUSTER_NAME
         DAEMON_NAME="execd"
      elif [ $hosttype = "loexecd" ]; then
         script_name=loexecd
         TMP_SGE_STARTUP_FILE=/tmp/loexecd.$$
         STARTUP_FILE_NAME=loexecd.$LO_CLUSTER_NAME
         S95NAME=S96loexecd.$LO_CLUSTER_NAME
         K03NAME=K02loexecd.$LO_CLUSTER_NAME
         DAEMON_NAME="loexecd"
      elif [ $hosttype = "ugerest" ]; then
         script_name=ugerest
         TMP_SGE_STARTUP_FILE="util/rctemplates/ugerest_systemd_template"
         STARTUP_FILE_NAME=sgeexecd.${SGE_CLUSTER_NAME}.service
         S95NAME=S96ugerest$SGE_CLUSTER_NAME
         K03NAME=K02ugerest$SGE_CLUSTER_NAME
         DAEMON_NAME="ugerest"
      else
      ...
      if [ $hosttype = "master" ]; then
         ...
         TMP_SGE_STARTUP_FILE=/tmp/loexecd.$$
         STARTUP_FILE_NAME=loexecd
         S95NAME=S96loexecd
         K03NAME=K02loexecd
         DAEMON_NAME="loexecd"
      elif [ $hosttype = "ugerest" ]; then
         TMP_SGE_STARTUP_FILE=/tmp/ugerest.$$
         STARTUP_FILE_NAME=ugerest
         S95NAME=S96ugerest
         K03NAME=K02ugerest
         DAEMON_NAME="ugerest"
      else
   ...
   if [ "$LO" = true ]; then
      SGE_STARTUP_FILE=$LO_ROOT/$LO_CELL/common/$STARTUP_FILE_NAME
   else
...
#-------------------------------------------------------------------------
# SetupRcScriptNames: Sets up RC script name variables
# $1 ... hosttype (master|bdb|execd|dbwriter|ugerest)
...
SetupRcScriptNames()
{
   ...
   if [ "$RC_FILE" = "systemctl" -a "$USE_SYSTEMD" = "true" ]; then
      ...
      if [ $hosttype = "master" ]; then
         ...
         script_name=ugerest
         TMP_SGE_STARTUP_FILE=/tmp/ugerest$$
         STARTUP_FILE_NAME=ugerest$SGE_CLUSTER_NAME
         S95NAME=S96ugerest$SGE_CLUSTER_NAME
         K03NAME=K02ugerest$SGE_CLUSTER_NAME
         DAEMON_NAME="ugerest"
      else
   ...
   if [ -z "$LO_ROOT" ]; then
      ...
      SGE_STARTUP_FILE=$LO_ROOT/$LO_CELL/common/$script_name
   fi
}
...
CheckRCfiles()
{
   ...
   rc_path=""
   # LSB, etc.
   if [ "$RC_FILE" = "lsb" -o "$RC_FILE" = "insserv-linux" -o "$RC_FILE" = "update-rc.d" -o "$RC_FILE" = "rc-update" -o "$RC_FILE" = "chkconfig" ]; then
      rc_path="$RC_PREFIX/$STARTUP_FILE_NAME"
   # Systemd
   elif [ "$RC_FILE" = "systemctl" -a "$USE_SYSTEMD" = "true" ]; then
      rc_path="$RC_PREFIX/${STARTUP_FILE_NAME}"
   # System V
...
CheckIfClusterNameAlreadyExists() 
{
   ...
   # NOTE: SMF is not supported in LO
   ...
   # Check if the launchd script does exist
   # NOTE: launchd is not supported in LO
   if [ -f /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeexecd.plist ] ||
      [ -f /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeqmaster.plist ] ||
      [ -f /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeshadowd.plist ]; then
      rc_res=1
   fi
   ...
   if [ $rc_res -eq 1 ]; then
      if [ $ret -eq 1 ]; then
         infotext_temp_msg="Detected a presence of old RC scripts for cluster >\$%s_CLUSTER_NAME=%s< as well!\n%s\n"
         ret=3
      else
         infotext_temp_msg="Specified cluster name >\$%s_CLUSTER_NAME=%s< resulted in the following conflict!\nDetected a presence of old RC scripts.\n%s\n"
         ret=2
      fi
      if [ "$AUTO" = "true" ]; then
         $INFOTEXT  -log "$infotext_temp_msg" $PRODUCT_SHORT $cluster_name $rc_path
      elif [ -z "$2" -o "$2" = "install" ]; then
         $INFOTEXT  "$infotext_temp_msg" $PRODUCT_SHORT $cluster_name $rc_path
      fi
   fi
...
SearchForExistingInstallations()
{
   ...
   SGE_CLUSTER_NAME=`cat $root_dir/$cell_dir/common/cluster_name 2>/dev/null`
   if [ -z "$SGE_CLUSTER_NAME" ]; then
      return
   fi
...
ProcessSGEClusterName()
{
   if [ "$LO" = true ]; then
      ...
      if [ "$LO_MASTER_PORT" = "" ]; then
         LO_MASTER_PORT=`./utilbin/$SGE_ARCH/getservbyname -number lo_master`
      fi
   else
      root_dir=$SGE_ROOT
      cell_dir=$SGE_CELL
   ...
   TMP_CLUSTER_NAME=`cat $root_dir/$cell_dir/common/cluster_name 2>/dev/null`
   ...
   while [ $done = false ]; do
      ...
      $CLEAR
      $INFOTEXT -u "\nUnique cluster name"
      $INFOTEXT "\nThe cluster name uniquely identifies a specific %s cluster.\n" \
                "The cluster name must be unique throughout your organization. The name \n" \
                " is not related to the %s cell.\n\n" \
                "The cluster name must start with a letter ([A-Za-z]), followed by letters, \n" \
                "digits ([0-9]), dashes ("-") or underscores ("_")." "$PRODUCT" $PRODUCT_SHORT
      $ECHO
   ...
   if [ \( "$1" = "bdb" -o "$1" = "qmaster" -o "$UPDATE" = "true" -o "$1" = "lomaster"  \) -a ! -f $root_dir/$cell_dir/common/cluster_name ]; then
      Makedir "$root_dir/$cell_dir/common"
      SafelyCreateFile $root_dir/$cell_dir/common/cluster_name 644 "$SGE_CLUSTER_NAME"
   fi

   LO_CLUSTER_NAME=$SGE_CLUSTER_NAME
   $ECHO
   $INFOTEXT "Your \$%s_CLUSTER_NAME: %s\n" $PRODUCT_SHORT $SGE_CLUSTER_NAME
   $INFOTEXT -log "Your \$%s_CLUSTER_NAME: %s" $PRODUCT_SHORT $SGE_CLUSTER_NAME
   $INFOTEXT -wait -auto $AUTO -n "Hit <RETURN> to continue >> "
   $CLEAR
}
...
GiveHints()
{
   ...
   while [ $done = false ]; do
      ...
      $INFOTEXT "\nYou should now enter the command:\n\n" \
                ...
                "   - \$%s_ROOT         (always necessary)\n" \
                "   - \$%s_CELL         (if you are using a cell other than >default<)\n" \
                "   - \$%s_CLUSTER_NAME (always necessary)\n" \
                "   - \$%sMASTER_PORT (if you haven't added the service >%s<)\n" \
                "   - \$%s_EXECD_PORT   (if you haven't added the service >%s<)\n" \
                "   - \$PATH/\$path       (to find the %s binaries)\n" \
                "   - \$MANPATH          (to access the manual pages)\n" \
                $root_dir/$cell_dir/common/settings.csh \
                $root_dir/$cell_dir/common/settings.sh $PRODUCT_SHORT $PRODUCT_SHORT \
                $PRODUCT_SHORT $MASTER_PREFIX $SGE_MASTER_NAME $PRODUCT_SHORT $SGE_EXECD_NAME "$PRODUCT"

      $INFOTEXT -wait -auto $AUTO -n "Hit <RETURN> to see where %s logs messages >> " "$PRODUCT"
      $CLEAR

      tmp_spool=`cat $root_dir/$cell_dir/common/bootstrap | grep qmaster_spool_dir | awk '{ print $2 }'`
      master_spool=`dirname $tmp_spool`

      if [ "$LO" = true ]; then
         SPOOL_TOKEN="lomaster"
         MASTER_STARTUP_FILE=$LO_ROOT/$LO_CELL/common/lomaster
         EXECD_STARTUP_FILE=$LO_ROOT/$LO_CELL/common/loexecd
         MESSAGES_FILE="/tmp/lo_messages"
      else
         SPOOL_TOKEN="qmaster"
         MASTER_STARTUP_FILE=$SGE_ROOT/$SGE_CELL/common/sgeqmaster
         EXECD_STARTUP_FILE=$SGE_ROOT/$SGE_CELL/common/sgeexecd
         MESSAGES_FILE="/tmp/sge_messages"
      fi

      $INFOTEXT -u "\n%s messages" "$PRODUCT"
      if [ "$SGE_ENABLE_SMF" = true -a "$LO" = false ]; then
         ...
         $INFOTEXT "\n%s messages can be found at:\n\n" \
                   "   %s (during %smaster startup)\n" \
                   "   /tmp/execd_messages   (during execution daemon startup)\n\n" "$PRODUCT" "$MESSAGES_FILE" $PRODUCT_SHORT_S
      fi
      $INFOTEXT "After startup the daemons log their messages in their spool directories.\n\n" \
                "   %smaster:     %s\n" \
                "   Exec daemon: <execd_spool_dir>/<hostname>/messages\n" $MASTER_PREFIX_S $master_spool/$SPOOL_TOKEN/messages

      $INFOTEXT -u "\n%s startup scripts" "$PRODUCT"
      $INFOTEXT "\n%s startup scripts can be found at:\n\n" \
                "   %s (%s)\n" \
                "   %s (execd)\n" "$PRODUCT" "$MASTER_STARTUP_FILE" "$SPOOL_TOKEN" "$EXECD_STARTUP_FILE"

      $INFOTEXT -auto $AUTO -ask "y" "n" -def "n" -n \
                "Do you want to see previous screen about using %s again (y/n) [n] >> " "$PRODUCT"
      if [ $? = 0 ]; then
         :
      else
         done=true
   ...
   if [ $QMASTER = install ]; then
      $CLEAR
      $INFOTEXT -u "\nYour %s %smaster installation is now completed" "$PRODUCT" "$MASTER_PREFIX_S"
      if [ "$LO" = false ]; then
      $INFOTEXT   "\nPlease now login to all hosts where you want to run an execution daemon\n" \
         ...
         $INFOTEXT   "\nYou may begin adding Univa Grid Engine Clusters or License\n" \
                     "Managers to your Univa License Orchestrator Cluster.  For more\n" \
                     "information refer to the Univa License Orchestrator Guide.\n\n" 
      fi
...
CreateMacOSXlaunchdMasterScript()
{
   if [ "$SGE_ARCH" != "darwin-x64" -a "$SGE_ARCH" != "darwin-x86" ]; then
      return
   fi

   # Print a warning if existing launchd script is overwritten
   if [ -f /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeqmaster.plist ]; then
      echo "Warning: Existing launchd script will be overwritten."
   fi

   # modify the sgeqmaster.plist according to current settings
   echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
<!DOCTYPE plist PUBLIC "-//Apple Computer//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version=\"1.0\">
<dict>
   <key>Label</key>
   <string>com.univa.gridengine.$SGE_CLUSTER_NAME.sgeqmaster</string>
   <key>Program</key>
   <string>$SGE_ROOT/bin/$SGE_ARCH/sge_qmaster</string>
   <key>RunAtLoad</key>
   <true/>
...
" > /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeqmaster.plist

   # register service
   launchctl load -w /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeqmaster.plist
}

CreateMacOSXlaunchdUGERESTScript()
{
   if [ "$SGE_ARCH" != "darwin-x64" -a "$SGE_ARCH" != "darwin-x86" ]; then
      return
   fi
   ...
   # Print a warning if existing launchd script is overwritten
   if [ -f /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.ugerest.plist ]; then
      echo "Warning: Existing launchd script will be overwritten."
   fi
...
<dict>
   <key>Label</key>
   <string>com.univa.gridengine.$SGE_CLUSTER_NAME.ugerest</string>
   <key>ProgramArguments</key>
   <array>
      <string>$SGE_ROOT/$SGE_CELL/common/ugerest</string>
      <string>start</string>
   </array>
   <key>RunAtLoad</key>
   <true/>
...
" > /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.ugerest.plist

   # register service
   launchctl unload /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.ugerest.plist 2> /dev/null > /dev/null
   launchctl load -w /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.ugerest.plist
}

#--------------------------------------------------------------------------
# Create for MacOSX the execd launchd script
#
CreateMacOSXlaunchdScript()
{
   if [ "$SGE_ARCH" != "darwin-x64" -a "$SGE_ARCH" != "darwin-x86" ]; then
      return
   fi
   ...
   # Print a warning if existing launchd script is overwritten
   if [ -f /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeexecd.plist ]; then
      echo "Warning: Existing launchd script will be overwritten."
   fi

   # modify the macosx_excecd.plist according to current settings
   echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
<!DOCTYPE plist PUBLIC "-//Apple Computer//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version=\"1.0\">
<dict>
   <key>Label</key>
   <string>com.univa.gridengine.$SGE_CLUSTER_NAME.sgeexecd</string>
   <key>Program</key>
   <string>$SGE_ROOT/bin/$SGE_ARCH/sge_execd</string>
   <key>RunAtLoad</key>
   <true/>
   <key>EnvironmentVariables</key>
   <dict><key>SGE_ROOT</key>
...
" > /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeexecd.plist

   launchctl load -w /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeexecd.plist
}
...
CreateSGEStartUpScripts()
{
   ...
   if [ "$LO" = true ]; then
      cluster_name=$LO_CLUSTER_NAME
      master_port=$LO_MASTER_PORT
      execd_port=$LO_EXECD_PORT
      SGE_STARTUP_FILE=$LO_ROOT/$LO_CELL/common/$STARTUP_FILE_NAME
   else
      cluster_name=$SGE_CLUSTER_NAME
      master_port=$SGE_QMASTER_PORT
      execd_port=$SGE_EXECD_PORT
      SGE_STARTUP_FILE=$SGE_ROOT/$SGE_CELL/common/$STARTUP_FILE_NAME
   fi
   ...
   if [ $create = true ]; then
      ...
      if [ "$RC_FILE" = "systemctl" -a "$USE_SYSTEMD" = "true" ]; then
         if [ $hosttype = "master" ]; then
            ...
            template1="util/rctemplates/sgemaster_systemd_template"
            svc_name="sgemaster.${cluster_name}"
         fi
         if [ $hosttype = "execd" ]; then
            template="util/rctemplates/sgeexecd_template"
            template1="util/rctemplates/sgeexecd_systemd_template"
            svc_name="sgeexecd.${cluster_name}"
         fi
      else	
      # use RC templates
         if [ $hosttype = "master" ]; then
            ...
            template="util/rctemplates/lomaster_template"
            svc_name="lomaster.${cluster_name}"
         elif [ $hosttype = "loexecd" ]; then
            template="util/rctemplates/loexecd_template"
            svc_name="loexecd.${cluster_name}"
         else
            template="util/rctemplates/sgeexecd_template"
            svc_name="sgeexecd.${cluster_name}"
         fi
      ...
      if [ "$master_port" != "" -a "$qmaster_service" != "true" ]; then
         Execute sed -e "s/=GENSGE_QMASTER_PORT/=$master_port/" \
                     ${TMP_SGE_STARTUP_FILE}.0 > $TMP_SGE_STARTUP_FILE.1
      else
         # ATTENTION: No line break for this eval call here !!!
         if [ "$LO" = true ]; then
            sed_param="s/^.*GENSGE_QMASTER_PORT.*LO_MASTER_PORT/unset LO_MASTER_PORT/"
         else
      ...
      if [ "$execd_port" != "" -a "$execd_service" != "true" ]; then
         Execute sed -e "s/=GENSGE_EXECD_PORT/=$execd_port/" \
                     ${TMP_SGE_STARTUP_FILE}.1 > $TMP_SGE_STARTUP_FILE
      else
         # ATTENTION: No line break for this eval call here !!!
         if [ "$LO" = true ]; then
            sed_param="s/^.*GENSGE_EXECD_PORT.*LO_EXECD_PORT/unset LO_EXECD_PORT/"
         else
      ...
      if [ "$RC_FILE" = "systemctl" -a "$USE_SYSTEMD" = "true" ]; then
         ...
         Execute sed -e "s%GENROOT%${SGE_ROOT_VAL}%g" \
		     -e "s%GENCELL%${SGE_CELL_VAL}%g" \
                     $template1 > ${SGE_STARTUP_FILE}.$SGE_CLUSTER_NAME.service
      else
      	 Execute $CHMOD 666 $TMP_SGE_STARTUP_FILE
      	 ExecuteAsAdmin $CP $TMP_SGE_STARTUP_FILE $SGE_STARTUP_FILE
      	 ExecuteAsAdmin $CHMOD a+x $SGE_STARTUP_FILE
      ...
      if [ $euid = 0 -a "$ADMINUSER" != default -a $QMASTER = "install" -a \( $hosttype = "master" -o $hosttype = "lomaster" \) ]; then
         AddDefaultManager root $ADMINUSER
         AddDefaultOperator $ADMINUSER
      elif [ $euid != 0 -a \( $hosttype = "master" -o $hosttype = "lomaster" \) ]; then
...
AddSGEStartUpScript()
{
   ...
   if [ -z "$cluster_name" ]; then
      $INFOTEXT "AddSGEStartUpScript error: expected %s_CLUSTER_NAME!!!" $PRODUCT_SHORT
      exit 1
   fi
...
#--------------------------------------------------------------------------
# Handle the installation of RC scripts on Windows Vista and later
#
# No input parameters
# Return Value: 1
# No output
...
{
   # No need to install a RC script on Vista, it can't start the Execd
   $INFOTEXT "\nOn Windows Vista, because of a bug in SUA, the RC scripts likely won't be able\n" \
             "to start the execd with sufficient permissions at boot time.\n" \
             "If this is the case, the execd will end itself and not run as daemon.\n"
}

#--------------------------------------------------------------------------
# Handle the uninstallation of RC scripts on Windows Vista and later
#
# No input parameters
# Return Value: 1
# No output
...
{
   # No need to install a RC script on Vista, it can't start the Execd
   $INFOTEXT "\nThe RC script should now be removed.\n"
}
...
InstallRcScript()
{
   ...
   # RC on shadow are in qmaster already
      ...
      ret=$?
      if [ "$AUTO" = "true" -a "$USE_SYSTEMD" = "true" -a "$ADD_TO_RC" = "false" ]; then
         SYSTEMD_ADD2RC=1
      else
         SYSTEMD_ADD2RC=$ret
      fi
      ...
      if [ $ret -eq 0 ]; then
         ...
         if [ "$DAEMON_NAME" = "execd" -a "$SGE_ARCH" = "win32-x86" -a "$version" -ge "5" ]; then
            InstallVistaRcScript
         fi
      fi
   fi
   ...
   # Systemd integration
   if [ "$RC_FILE" = "systemctl" -a "$USE_SYSTEMD" = "true" ]; then
      echo cp $SGE_STARTUP_FILE.$SGE_CLUSTER_NAME.service $RC_PREFIX/${STARTUP_FILE_NAME}
      echo $SYSTEMCTL enable ${STARTUP_FILE_NAME}
      Execute cp $TMP_SGE_STARTUP_FILE $RC_PREFIX/${STARTUP_FILE_NAME}
      if [ -z "${SGE_ROOT_VAL}" ]; then
         Execute sed -e 's%GENROOT%'${SGE_ROOT}'%g' \
                     -e 's%GENCELL%'${SGE_CELL}'%g' \
                     -i $RC_PREFIX/${STARTUP_FILE_NAME}
      else
         Execute sed -e 's%GENROOT%'${SGE_ROOT_VAL}'%g' \
	             -e 's%GENCELL%'${SGE_CELL_VAL}'%g' \
                     -i $RC_PREFIX/${STARTUP_FILE_NAME}
      fi
      Execute $SYSTEMCTL enable ${STARTUP_FILE_NAME} # removed $RC_PREFIX
   ...
   # If we have a Red Hat version with chkconfig we use this tool
   elif [ "$RC_FILE" = "chkconfig" ]; then
      echo cp $SGE_STARTUP_FILE $RC_PREFIX/$STARTUP_FILE_NAME
      echo /sbin/chkconfig --add $STARTUP_FILE_NAME
      Execute cp $SGE_STARTUP_FILE $RC_PREFIX/$STARTUP_FILE_NAME
      Execute /sbin/chkconfig --add $STARTUP_FILE_NAME
   ...
   # and make a link in $RC_PREFIX/rc2.d to $RC_PREFIX/init.d
      ...
      # Old MacOs integration scripts for non darwin-x64 hosts
...
# these files/directories might exist and need to be backed up
BUP_COMMON_FILE_LIST="accounting act_qmaster bootstrap cluster_name host_aliases qtask"
BUP_COMMON_FILE_LIST="$BUP_COMMON_FILE_LIST sge_aliases settings.csh settings.sh sge_ar_request"
BUP_COMMON_FILE_LIST="$BUP_COMMON_FILE_LIST sge_qquota sge_qrstat sge_qstat sge_request sgepasswd"
BUP_COMMON_FILE_LIST="$BUP_COMMON_FILE_LIST shadow_masters"
if [ "$LO" = true ]; then
   BUP_COMMON_FILE_LIST="$BUP_COMMON_FILE_LIST loexecd lomaster"
else
   BUP_COMMON_FILE_LIST="$BUP_COMMON_FILE_LIST sgeexecd sgemaster"
fi
BUP_COMMON_DIR_LIST="sgeCA"
BUP_SPOOL_FILE_LIST="arseqnum jobseqnum"
BUP_SPOOL_DIR_LIST=""

BUP_CLASSIC_COMMON_FILE_LIST="configuration sched_configuration"
BUP_CLASSIC_DIR_LIST="local_conf"
BUP_CLASSIC_SPOOL_FILE_LIST="admin_hosts advance_reservations calendars centry ckpt cqueues "
BUP_CLASSIC_SPOOL_FILE_LIST="$BUP_CLASSIC_SPOOL_FILE_LIST exec_hosts hostgroups jclasses managers operators pe projects "
BUP_CLASSIC_SPOOL_FILE_LIST="$BUP_CLASSIC_SPOOL_FILE_LIST qinstances resource_quotas schedd submit_hosts usermapping users "
BUP_CLASSIC_SPOOL_FILE_LIST="$BUP_CLASSIC_SPOOL_FILE_LIST usersets vinstances zombies"
if [ "$LO" = true ]; then
   BUP_CLASSIC_SPOOL_FILE_LIST="$BUP_CLASSIC_SPOOL_FILE_LIST license_servers uge_clusters"
fi

# these files actually exist and have been prepared for backup
BUP_BACKUP_FILE_DIR_LIST=""
...
BackupConfig()
{
   ...
   $INFOTEXT -u "${PRODUCT_SHORT} Configuration Backup"
   $INFOTEXT -n "\nThis feature does a backup of all configuration you made\n" \
                "within your cluster."
                if [ $AUTO != "true" ]; then
                   SGE_ROOT=`pwd`
                fi
   $INFOTEXT -n "\nPlease enter your ${PRODUCT_SHORT}_ROOT directory.\nDefault: [%s]" $SGE_ROOT
                SGE_ROOT=`Enter $SGE_ROOT`
   $INFOTEXT -n "\nPlease enter your ${PRODUCT_SHORT}_CELL name. Default: [default]"
                if [ $AUTO != "true" ]; then
                   SGE_CELL=`Enter default`
                fi

   $INFOTEXT -log "${PRODUCT_SHORT}_ROOT: %s" $SGE_ROOT
   $INFOTEXT -log "${PRODUCT_SHORT}_CELL: %s" $SGE_CELL
...
RestoreConfig()
{
   ...
   $INFOTEXT -u "${PRODUCT_SHORT} Configuration Restore"
   ...
   if [ $? = 0 ]; then
      ...
      cd $SGE_ROOT
      bup_file="/tmp/bup_tmp_$DATE"
      DoRestoreConfig $bup_file
      rm -fR $bup_file
   else
      loop_stop="false"
...
DoRestoreConfig()
{
   ...
   # spooling method independent restore
   ...
   for f in $BUP_SPOOL_DIR_LIST; do
      if [ -d $bup_file/$f ]; then
         ExecuteAsAdmin $CPR $bup_file/$f $master_spool
      fi
   done

   # spooling method dependent restore
   case $spooling_method in
      classic)
         ...
         for f in $BUP_CLASSIC_SPOOL_FILE_LIST; do
            if [ -f $bup_file/$f -o -d $bup_file/$f ]; then
               if [ -f $master_spool/$f -o -d $master_spool/$f ]; then
                  #move the old configuration to keep backup
                  ExecuteAsAdmin mv -f $master_spool/$f $master_spool/$f.bak
               fi
               ExecuteAsAdmin $CPR $bup_file/$f $master_spool
            fi
         done
      ...
      *)
         DoRestore_$spooling_method $bup_file
         ;;
...
RemoveRcScript()
{
   ...
   # Use chkconfig --del for deleting services
   elif [ "$RC_FILE" = "chkconfig" ]; then
      echo /sbin/chkconfig --del $STARTUP_FILE_NAME
      echo rm -f $RC_PREFIX/$STARTUP_FILE_NAME
      Execute /sbin/chkconfig --del $STARTUP_FILE_NAME
      Execute rm -f $RC_PREFIX/$STARTUP_FILE_NAME
   # If we have System V we need to put the startup script to $RC_PREFIX/init.d
   # and make a link in $RC_PREFIX/rc2.d to $RC_PREFIX/init.d
   elif [ "$RC_FILE" = "update-rc.d" ]; then
   # update-rc.d uninstall
      echo "update-rc.d -f $STARTUP_FILE_NAME remove"
      echo "rm -f $RC_PREFIX/$STARTUP_FILE_NAME"
      Execute update-rc.d -f $STARTUP_FILE_NAME remove
      Execute rm -rf $RC_PREFIX/$STARTUP_FILE_NAME
   elif [ "$RC_FILE" = "sysv_rc" ]; then
   ...
   # systemd uninstall
      ...
      echo $SYSTEMCTL disable ${STARTUP_FILE_NAME}
      echo  rm -f $RC_PREFIX/${STARTUP_FILE_NAME}
      Execute $SYSTEMCTL disable ${STARTUP_FILE_NAME} # removed $RC_PREFIX
      Execute rm -f $RC_PREFIX/${STARTUP_FILE_NAME}
   elif [ "$RC_FILE" = "insserv-linux" ]; then
      ...
      if [ $hosttype = "master" ]; then
         # disable service
         echo "launchctl unload -w /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeqmaster.plist"
         launchctl unload -w /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeqmaster.plist
         # remove service
         echo  rm -f /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeqmaster.plist
         Execute rm -f /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeqmaster.plist
      elif [ $hosttype = "execd" ]; then
           # disable service
         echo "launchctl unload -w /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeexecd.plist"
         launchctl unload -w /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeexecd.plist
         # remove service
         echo  rm -f /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeexecd.plist
         Execute rm -f /Library/LaunchDaemons/com.univa.gridengine.$SGE_CLUSTER_NAME.sgeexecd.plist
      fi 
   elif [ "$RC_FILE" = "SGE" ]; then
      if [ -z "$v61" ]; then
         RC_DIR="$RC_DIR.$SGE_CLUSTER_NAME"
...
BackupCheckBootStrapFile()
{
   if [ -f $SGE_ROOT/$SGE_CELL/common/bootstrap ]; then
      ...
      spooling_params=`cat $SGE_ROOT/$SGE_CELL/common/bootstrap | grep "spooling_params" | tr -s '[:space:]' ' ' | cut -d ' ' -f 2-`
      ...
      case $spooling_method in
         berkeleydb)
            ...
            spooling_params=`echo $spooling_params | sed "s, $,,"`
            if [ `echo $spooling_params | cut -d: -f2` = "$spooling_params" ]; then
               is_rpc=0
               db_home=$spooling_params
            else
               is_rpc=1
               BDB_SERVER=`echo $spooling_params | cut -d":" -f1`
               BDB_SERVER=`$SGE_UTILBIN/gethostbyname -aname $BDB_SERVER`
               BDB_BASEDIR=`echo $spooling_params | cut -d":" -f2`
         ...
         postgres)
            for param in $spooling_params; do
               param_name=`echo $param | cut -f 1 -d =`
               param_value=`echo $param | cut -f 2 -d =`
               eval pg_$param_name=$param_value
            done
            ;;
      esac
   else
...
CreateTarArchive()
{
   if [ $TAR = "true" ]; then
      ...
      if [ $? -eq 0 ]; then
         TAR=$TAR" -cvf"
         if [ "$spooling_method" = "berkeleydb" -o "$spooling_method" = "postgres" ]; then
            ExecuteAsAdmin $TAR $bup_file $DATE.dump $BUP_BACKUP_FILE_DIR_LIST
         else
            ExecuteAsAdmin $TAR $bup_file $BUP_BACKUP_FILE_DIR_LIST
         fi
      ...
      cd $backup_dir
      RMF="rm -fR"
      ExecuteAsAdmin $RMF $DATE.dump.tar $DATE.dump $BUP_BACKUP_FILE_DIR_LIST
...
DoBackup()
{
   ...
   # backup the files / directories common to all spooling methods
   for f in $BUP_COMMON_FILE_LIST; do
      if [ -f $SGE_ROOT/$SGE_CELL/common/$f ]; then
         BUP_BACKUP_FILE_DIR_LIST="$BUP_BACKUP_FILE_DIR_LIST $f"
   ...
   for f in $BUP_SPOOL_DIR_LIST; do
      if [ -d $master_spool/$f ]; then
         BUP_BACKUP_FILE_DIR_LIST="$BUP_BACKUP_FILE_DIR_LIST $f"
         ExecuteAsAdmin $CPFR $master_spool/$f $backup_dir
      fi
   done

   # backup spooling method dependent files
   case $spooling_method in
      classic)
         for f in $BUP_CLASSIC_COMMON_FILE_LIST; do
            if [ -f $SGE_ROOT/$SGE_CELL/common/$f ]; then
...
RestoreCheckBootStrapFile()
{
   ...
   if [ -f $BACKUP_DIR/bootstrap ]; then
      ...
      spooling_params=`cat $BACKUP_DIR/bootstrap | grep "spooling_params" | tr -s '[:space:]' ' ' | cut -d ' ' -f 2-`
      ...
      MASTER_PORT=`cat $BACKUP_DIR/${PRODUCT_SHORT_S}master | grep "SGE_QMASTER_PORT=" | head -1 | awk '{ print $1 }' | cut -d"=" -f2 | cut -d";" -f1`

      ACT_QMASTER=`cat $BACKUP_DIR/act_qmaster`

      $SGE_BIN/qping -info $ACT_QMASTER $MASTER_PORT $MASTER_NAME 1 > /dev/null 2>&1
      ret=$?

      while [ $ret = 0 ]; do
         $INFOTEXT -n "\nFound a running $MASTER_NAME on your masterhost: %s\nPlease, check this and " \
                      "make sure, that the daemon is down during the restore!\n\n" $ACT_QMASTER
         $INFOTEXT -n -wait "Shutdown $MASTER_NAME and hit, <ENTER> to continue, or <CTRL-C> to stop\n" \
...
CopyCA()
{
   if [ "$CSP" = "false" -a "$WINDOWS_SUPPORT" = "false" -a "$1" != "copyonly" ]; then
      return 1
   fi
   ...
   $INFOTEXT -u "Installing %s in CSP mode" $PRODUCT_SHORT
   if [ "$hosttype" = "copyonly" ]; then
      ...
      $INFOTEXT "\nInstalling %s in CSP mode needs to copy the cert\n" \
                "files to each %s host. This can be done by script!\n" $PRODUCT_SHORT $out_text
      $INFOTEXT "To use this functionality, it is recommended, that user root\n" \
                "may do rsh/ssh to the %s host, without being asked for a password!\n" $out_text
...
CopyCaToHostType()
{
   ...
   if [ "$1" = "admin" ]; then
      cmd=`$SGE_BIN/$conf_cmd -sh`
   elif [ "$1" = "submit" ]; then
      cmd=`$SGE_BIN/$conf_cmd -ss`
   elif [ "$1" = "remote" ]; then
   ...
   for RHOST in $cmd; do
      if [ "$RHOST" != "$HOST" ]; then
         ...
         if [ "$?" = 0 ]; then
            ...
            lowercase_product=`echo ${PRODUCT_SHORT} | tr '[:upper:]' '[:lower:]'`
            ca_root="/var/${lowercase_product}CA"
            echo "mkdir ${ca_root} > /dev/null 2>&1" | $SHELL_NAME $RHOST /bin/sh &
            if [ "$SGE_QMASTER_PORT" = "" ]; then
               ca_port_dir="${ca_root}/${lowercase_product}_qmaster"
               $COPY_COMMAND -pr $HOST:${ca_port_dir} $RHOST:${ca_root}
            else
               ca_port_dir="${ca_root}/port$SGE_QMASTER_PORT"
               $COPY_COMMAND -pr $HOST:${ca_port_dir} $RHOST:${ca_root}
            fi
            if [ "$?" = 0 ]; then
               $INFOTEXT "Setting ownership to adminuser %s" $ADMINUSER
               $INFOTEXT -log "Setting ownership to adminuser %s" $ADMINUSER
               echo "chown $ADMINUSER ${ca_port_dir}" | $SHELL_NAME $RHOST /bin/sh &
               echo "chown -R $ADMINUSER ${ca_port_dir}/$SGE_CELL" | $SHELL_NAME $RHOST /bin/sh &
               for dir in `ls ${ca_port_dir}/$SGE_CELL/userkeys`; do
                  echo "chown -R $dir ${ca_port_dir}/$SGE_CELL/userkeys/$dir" | \
                       $SHELL_NAME $RHOST /bin/sh &
               done
            else
...
CheckPortsCollision()
{
   ...
   if [ $check_val = "lo_master" ]; then
      ENV_VAR="$LO_MASTER_PORT"
   fi

   if [ $check_val = "lo_execd" ]; then
      ENV_VAR="$LO_EXECD_PORT"
   fi
...
ManipulateOneDaemonType()
{
   ...
   if [ "$LO" = true ]; then
      ...
      if [ -f $LO_ROOT/$LO_CELL/common/bootstrap ]; then
         ...
         $INFOTEXT "\nObviously there was no License Orchestrator installation for this cell yet. The file\n\n" \
                   "   %s\n\ndoes not exist. Exit." \$LO_ROOT/$LO_CELL/common/bootstrap
         exit 1
      fi

      if [ -f $LO_ROOT/$LO_CELL/common/settings.sh ]; then
         . $LO_ROOT/$LO_CELL/common/settings.sh
      else
         $INFOTEXT "\nThe file\n\n" \
   ...
   if [ "$ALL_RC" = true ]; then
	 ...
	 list=""
         ...
         if [ -f "$LO_ROOT/$LO_CELL/common/shadow_masters" ]; then
            list=`cat "$LO_ROOT/$LO_CELL/common/shadow_masters"`
         elif [ -f "$LO_ROOT/$LO_CELL/common/act_qmaster" ]; then
            list=`cat "$LO_ROOT/$LO_CELL/common/act_qmaster"`
         fi
	 start_cmd="$LO_ROOT/$LO_CELL/common/lomaster"
	 ;;
      loexecd)
         list=`cat "$LO_ROOT/$LO_CELL/common/act_qmaster"`
	 start_cmd="$LO_ROOT/$LO_CELL/common/loexecd"
    ;;
      execd)
	 ...
	 ;;
      bdb)
         if [ "$LO" = true ]; then
            list=`cat $LO_ROOT/$LO_CELL/common/bootstrap | grep "spooling_params" | awk '{ print $2}' 2>/dev/null`
            start_cmd="$LO_ROOT/$LO_CELL/common/lobdb"
         else
   ...
   if [ "$LO" = true ]; then
      rc_cmd=". $LO_ROOT/$LO_CELL/common/settings.sh ; . $LO_ROOT/util/arch_variables ; \
              . $LO_ROOT/util/install_modules/inst_common.sh ; \
              . $LO_ROOT/util/install_modules/inst_qmaster.sh ; \
              . $LO_ROOT/util/install_modules/inst_berkeley.sh ; \
              . $LO_ROOT/util/install_modules/inst_execd.sh ; \
               AUTO=true ; export AUTO ; SGE_ENABLE_SMF=false ; export SGE_ENABLE_SMF ; \
               euid=$euid ; cd $LO_ROOT ; BasicSettings ; SetUpInfoText ; CheckForSMF ; "
   else
      rc_cmd=". $SGE_ROOT/$SGE_CELL/common/settings.sh ; . $SGE_ROOT/util/arch_variables ; \
              ...
              . $SGE_ROOT/util/install_modules/inst_berkeley.sh ; \
              . $SGE_ROOT/util/install_modules/inst_execd.sh ; \
               AUTO=true ; export AUTO ; SGE_ENABLE_SMF=true ; export SGE_ENABLE_SMF ; \
               euid=$euid ; cd $SGE_ROOT ; BasicSettings ; SetUpInfoText ; CheckForSMF ; "
   ...
   if [ "$LO" = true ]; then
      if [ "$DEL_EXECD_SPOOL" = true ]; then #DELETE EXECD SPOOL DIRS
         cmd=". $LO_ROOT/$LO_CELL/common/settings.sh ; . $LO_ROOT/util/arch_variables ; \
              . $LO_ROOT/util/install_modules/inst_common.sh ; \
              cd $LO_ROOT && RemoteExecSpoolDirDelete"
         $INFOTEXT -u "Initializing all local execd spool directories:"
         DoRemoteActionForHosts "$list" default "$cmd"
      fi
   else
      if [ "$DEL_EXECD_SPOOL" = true ]; then #DELETE EXECD SPOOL DIRS
         cmd=". $SGE_ROOT/$SGE_CELL/common/settings.sh ; . $SGE_ROOT/util/arch_variables ; \
              . $SGE_ROOT/util/install_modules/inst_common.sh ; \
      ...
      if [ "$UPDATE_WIN" = true ]; then      #UPDATE WINDOWS HELPER SERVICE ON ALL WINDOWS EXECDs
         cmd=". $SGE_ROOT/$SGE_CELL/common/settings.sh ; . $SGE_ROOT/util/arch_variables ; \
              . $SGE_ROOT/util/install_modules/inst_common.sh ; \
              . $SGE_ROOT/util/install_modules/inst_execd.sh ; \
              cd $SGE_ROOT ; AUTO=true ; ECHO=echo ;BasicSettings ; \
              SetUpInfoText ; SAVED_PATH=$PATH ; SetupWinSvc update"
...
VerifyExecdBinaries()
{
  ...
  if [ "$LO" = true ]; then
    list=`cat "$LO_ROOT/$LO_CELL/common/act_qmaster"`
    SGE_VERSION=`$LO_ROOT/bin/$SGE_ARCH/lomaster -help | head -1`
  else
    list=`$SGE_BIN/qconf -sel`
    SGE_VERSION=`$SGE_BIN/sge_qmaster -help | head -1`
  fi
  ...
  if [ "$LO" = true ]; then
    cmd=". $LO_ROOT/$LO_CELL/common/settings.sh ; . $LO_ROOT/util/arch_variables ; \
        . $LO_ROOT/util/install_modules/inst_common.sh ; \
        cd $LO_ROOT && RemoteExecVerifyExecdBinaries "\"$SGE_VERSION\"""
    $INFOTEXT -u "Verifying binaries:"
    DoRemoteActionForHosts "$list" default "$cmd"
  else
    cmd=". $SGE_ROOT/$SGE_CELL/common/settings.sh ; . $SGE_ROOT/util/arch_variables ; \
        . $SGE_ROOT/util/install_modules/inst_common.sh ; \
        cd $SGE_ROOT && RemoteExecVerifyExecdBinaries "\'$SGE_VERSION\'""
    $INFOTEXT -u "Verifying binaries:"
    DoRemoteActionForHosts "$list" default "$cmd"
  fi
...
RemoteExecSpoolDirDelete() 
{
   ...
   if [ -z "$LO_ROOT" ]; then
      ...
      global_dir=`loconf -sconf 2>&1 | grep execd_spool_dir | awk '{ print $2}'`
      local_dir=`loconf -sconf $HOST 2>&1 | grep execd_spool_dir | awk '{ print $2}'` 
   fi

   if [ -z "$local_dir" ]; then 
      if [ -z "$global_dir" ]; then
	  	$INFOTEXT -u "Cannot process host $HOST (Invalid execd_spool_dir parameter)"	
	  	return
      fi
...
RemoteExecVerifyExecdBinaries()
{
  ...
  #exclude: qmake, qtcsg, sgepasswd (no -help output)

  if [ "$LO" = true ]; then
      BINFILES="lo_execd lo_master lo_shadowd lo_shepherd loacct loconf lodel \
                lostat losub loquota lomod"
      BINDIR="$LO_ROOT/bin/$SGE_ARCH"
  elif [ "$SGE_ARCH" = "win32-x86" ]; then
      BINFILES="sge_coshepherd sge_execd sge_shepherd  \
                qacct qalter qconf qdel qhold qhost qlogin \
                qmake qmod qresub qrls qrsh qselect qsh \
                qstat qsub qtcsh qping qquota qloadsensor.exe"
      BINDIR=$SGE_BIN
  ...
  if [ -z "$SGE_VERSION" ]; then
    if [ "$LO" = true ]; then
      SGE_VERSION=`$LO_ROOT/bin/$SGE_ARCH/lomaster -help | head -1`
    else
      SGE_VERSION=`$BINDIR/sge_qmaster -help | head -1`
    fi
  fi
  ...
  for filename in $BINFILES; do
    ...
    if [ ! -f $BINDIR/$filename ]; then
        $INFOTEXT "Missing binary $filename on host $HOST" 
    fi

    #check if file has correct version
    BINVER=`$BINDIR/$filename -help 2> /dev/null | head -1`
    `echo "$BINVER" | grep -q "$SGE_VERSION"`
    if [ $? -ne 0 ]; then
       $INFOTEXT "Wrong version of binary $filename on host $HOST (is $BINVER, should be $SGE_VERSION)" 
    fi
  done
}
...
CreateCopyOfBackup()
{
   original_backup_dir="${1:?Missing original backup dir}"
   current_date="${2:?Missing current date}"

   if [ "$LO" = true ]; then
      copy_dir="/tmp/lo_backup_$current_date"
   else
      copy_dir="/tmp/sge_backup_$current_date"
   fi
   if [ ! -d "$copy_dir" ]; then
      mkdir -p "$copy_dir"
   else
      $INFOTEXT "Directory $copy_dir already exists!"
      exit 1
   fi

   cp -fR "${original_backup_dir}"/* "$copy_dir"

   # Make files readable for all
   chmod -R g+r,o+r "$copy_dir"/*
