...
GetCell()
{
   ...
   if [ $AUTO = true ]; then
    ...
    if [ -f $root_dir/$SGE_CELL/common/bootstrap -a "$QMASTER" = "install" ]; then
   ...
   while [ $is_done = "false" ]; do 
      $CLEAR
      $INFOTEXT -u "\n%s cells" "$PRODUCT"
      if [ "$SGE_CELL" = "" -a "$LO" = false ]; then
         SGE_CELL=default
      elif [ -n "$LO_CELL" -a "$LO" = true ]; then
         SGE_CELL=$LO_CELL
      elif [ -z "$LO_CELL" -a "$LO" = true ]; then
         SGE_CELL=default
      fi

      $INFOTEXT -n "\n%s supports multiple cells.\n\n" \
                   "If you are not planning to run multiple %s clusters or if you don't\n" \
                   "know yet what is a %s cell it is safe to keep the default cell name\n\n" \
                   "   default\n\n" \
                   "If you want to install multiple cells you can enter a cell name now.\n\n" \
                   "The environment variable\n\n" \
                   "   \$%s_CELL=<your_cell_name>\n\n" \
                   "will be set for all further %s commands.\n\n" \
                   "Enter cell name [%s] >> " "$PRODUCT" "$PRODUCT" "$PRODUCT" $PRODUCT_SHORT "$PRODUCT" $SGE_CELL
      INP=`Enter $SGE_CELL`
      eval SGE_CELL=$INP
      SGE_CELL_VAL=`eval echo $SGE_CELL`
      if [ "$QMASTER" = "install" ]; then
         if [ -d $root_dir/$SGE_CELL/common ]; then
            $CLEAR
            ...
            if [ $? = 0 ]; then
               ...
               with_bdb=0
               if [ ! -f $root_dir/$SGE_CELL/common/bootstrap -a -f $root_dir/$SGE_CELL/common/sgebdb ]; then
               ...
               sel_ret=$?
               if [ "$LO" = true ]; then
                  SearchForExistingInstallations "lomaster shadowd loexecd dbwriter"
               else
               ...
               if [ $sel_ret = 0 -a $with_bdb = 0 ]; then
                  ...
                  ExecuteAsAdmin rm -f $root_dir/$SGE_CELL_VAL/common/bootstrap
                  ExecuteAsAdmin rm -f $root_dir/$SGE_CELL_VAL/common/cluster_name
               elif [ $sel_ret -ne 0 ]; then
                  $INFOTEXT "Deleting directory \"%s\" now!" $SGE_ROOT/$SGE_CELL_VAL
                  Removedir $root_dir/$SGE_CELL_VAL
               fi
               if [ $sel_ret = 0 ]; then
                  Overwrite="true"
               fi
...
GetQmasterSpoolDir()
{
   if [ $AUTO = true ]; then
      ...
      $INFOTEXT -log "Using >%s< as %sMASTER_SPOOL_DIR." "$QMDIR" $MASTER_PREFIX
      #If directory exists and has files, we exit the auto installation
      if [ -d "$QMDIR" ]; then
         if [ `ls -1 "$QMDIR" | wc -l` -gt 0 ]; then
            $INFOTEXT -log "Specified %smaster spool directory \"$QMDIR\" is not empty!" $MASTER_PREFIX_S
      ...
      while [ $done = false ]; do
         $CLEAR
         $INFOTEXT -u "\n%s master spool directory" "$PRODUCT"
         $INFOTEXT "\nThe %smaster spool directory is the place where the %smaster daemon stores\n" \
                   "the configuration and the state of the queuing system.\n\n" $MASTER_PREFIX_S $MASTER_PREFIX_S

         if [ $euid = 0 ]; then
            if [ $ADMINUSER = default ]; then
               $INFOTEXT "User >root< on this host must have read/write access to the %smaster\n" \
                         "spool directory.\n" $MASTER_PREFIX_S
            else
               $INFOTEXT "The admin user >%s< must have read/write access\n" \
                      "to the master spool directory.\n" $ADMINUSER
         fi
      else
         $INFOTEXT "Your account on this host must have read/write access\n" \
                      "to the %smaster spool directory.\n" $MASTER_PREFIX_S
         fi

      if [ "$LO" = true ]; then
         DEFAULT_SPOOL_DIR="$SGE_ROOT_VAL/$SGE_CELL_VAL/spool/lomaster"
      else
         DEFAULT_SPOOL_DIR="$SGE_ROOT_VAL/$SGE_CELL_VAL/spool/qmaster"
      fi

      $INFOTEXT -n "If you will install shadow master hosts or if you want to be able to start\n" \
                      "the %smaster daemon on other hosts (see the corresponding section in the\n" \
                      "%s Installation and Administration Manual for details) the account\n" \
                   "on the shadow master hosts also needs read/write access to this directory.\n\n" \
                      "Enter a %smaster spool directory [%s] >> " $MASTER_PREFIX_S "$PRODUCT" $MASTER_PREFIX_S "$DEFAULT_SPOOL_DIR"
         QMDIR=`Enter "$DEFAULT_SPOOL_DIR"`
      done=true
      #If directory is not empty need now require a new one
      if [ -d "$QMDIR" ]; then
         if [ `ls -1 "$QMDIR" | wc -l` -gt 0 ]; then
               $INFOTEXT "Specified %smaster spool directory is not empty!" $MASTER_PREFIX_S
               $INFOTEXT -auto $AUTO -ask "y" "n" -def "y" -n \
                  "Do you want to select another %smaster spool directory (y/n) [y] >> " $MASTER_PREFIX_S
               if [ $? = 0 ]; then
                  done=false
               fi
            fi
   ...
   $INFOTEXT -wait -auto $AUTO -n "\nUsing %smaster spool directory >%s<. \nHit <RETURN> to continue >> " $MASTER_PREFIX_S "$QMDIR"
   export QMDIR
}
...
{
   if [ $AUTO = true ]; then
      READER_THREADS="$SGE_READER_THREADS"
      $INFOTEXT -log "Using >%s< as number of read-only threads." "$READER_THREADS" 
      if [ \( "$READER_THREADS" -lt 0 \) -o \( "$READER_THREADS" -gt 64 \) ]; then
         $INFOTEXT -log "Value for the number of worker threads is not in range between 0 and 64"
         MoveLog
         exit 1
      fi     
   else
      ...
      while [ $done = false ]; do
         ...
         $INFOTEXT -u "\nRead-Only Component"
         $ECHO
         $INFOTEXT "Univa Grid Engine allows to activate a read-only component as part of the\n" \
                   "qmaster process. This component can answer certain requests (read-only\n" \
                   "requests like qstat, qhost, ...) in parallel with other activities of qmaster\n" \
                   "(read-write requests like configuration changes, scheduling of jobs, ...) at\n" \
                   "the same point in time.\n\n"
         $INFOTEXT "An enabled read-only component will improve the responsiveness of qmaster\n" \
                   "and also the cluster throughput compared to previous versions of Grid Engine\n" \
                   "but it requires that dependent Univa Grid Engine commands are executed in a\n" \
                   "session. Consult the Univa Grid Engine documentation for more information\n\n"
         $INFOTEXT "How many reader threads should be started in the read-only component?\n" \
                   "0 will disable the read-only component. Higher numbers increase\n" \
                   "responsiveness of qmaster but increase also peak memory requirements."
         $INFOTEXT -n "Specify a value between 0 and 64. [%s] >> " "2"

         SGE_READER_THREADS=`Enter "2"`

         if [ \( $SGE_READER_THREADS -lt 0 \) -o \( $SGE_READER_THREADS -gt 64 \) ]; then
            $INFOTEXT -wait "Specified value is not a number between 0 and 64"
         else
            $INFOTEXT -wait -auto $AUTO -n "\nRead-only component will be started with >%s< threads.\nHit <RETURN> to continue >> " $SGE_READER_THREADS
            export SGE_READER_THREADS
            done=true
         fi
      done
   fi
...
SetPermissions()
{
   ...
   if [ $ret != 0 ]; then
      ...
      $INFOTEXT "\nWe can't set file permissions on this machine, because user root\n" \
                  ...
                  "   # \$%s_ROOT/util/setfileperm.sh \$%s_ROOT\n\n" $PRODUCT_SHORT $PRODUCT_SHORT
      ...
      $CLEAR
      $INFOTEXT -u "\nVerifying and setting file permissions"
      $INFOTEXT "\nWe may now verify and set the file permissions of your %s\n" \
                "distribution.\n\n" \
   ...
   if [ $ret = 0 ]; then
      ...
      if [ "$LO" = true ]; then
         util/setfileperm.sh -auto -lo $LO_ROOT
      else
...
SetSpoolingOptionsDynamic()
{
   ...
   if [ "$AUTO" = "true" ]; then
      if [ "$SPOOLING_METHOD" != "berkeleydb" -a "$SPOOLING_METHOD" != "classic" -a "$SPOOLING_METHOD" != "postgres" ]; then
         SPOOLING_METHOD="$suggested_method"
      fi
   else
      if [ "$BERKELEY" = "install" ]; then
         SPOOLING_METHOD="$suggested_method"
      else
         $INFOTEXT -n "Your binaries are compiled to link the spooling libraries\n" \
                      "during runtime (dynamically). So you can choose between Berkeley DB \n" \
                      "spooling, Classic spooling and Postgres spooling method."
   ...
   case $SPOOLING_METHOD in 
      ...
      berkeleydb)
         ...
         ;;
      postgres)
         if [ "$HAVE_POSTGRES_SPOOLING" = "true" ]; then
            SetSpoolingOptionsPostgres
         else
            $INFOTEXT "\nPostgres spooling is only supported in Univa Grid Engine. Exit."
            $INFOTEXT -log "\nPostgres spooling is only supported in Univa Grid Engine. Exit."
            MoveLog
            exit 1
         fi
...
SelectHostNameResolving()
{
   if [ $AUTO = "true" ]; then
     ...
     $CLEAR
     $INFOTEXT -u "\nSelect default %s hostname resolving method" "$PRODUCT"
...
SetProductMode()
{
   ...
   if [ $CSP = true ]; then
      ...
      aix* | hp11*)
         ...
         *)
          if [ "$LO" = true ]; then
             SEC_COUNT=`strings $SGE_BIN/lo_master | grep "AIMK_SECURE_OPTION_ENABLED" | wc -l`
          else
      ...
      if [ $SEC_COUNT -ne 1 ]; then
         $INFOTEXT "\n>%smaster< binary is not compiled with >-secure< option!\n"
         $INFOTEXT -wait -auto $AUTO -n "Hit <RETURN> to cancel the installation >> " $MASTER_PREFIX_S
         exit 1
      else
         CSP_PREFIX="csp"
      fi  
...
PrintBootstrap()
{
   ...
   if [ "$LO" = true ]; then
      $ECHO "reader_threads          0"
   else
      $ECHO "reader_threads          $SGE_READER_THREADS"
   fi
...
PrintConf()
{
   ...
   $ECHO "xterm                  $XTERM"
   if [ "$LO" = true ]; then
      $ECHO "load_sensor            $SGE_ROOT_VAL/util/license_sensor"
   else
   ...
   $ECHO "projects               none"
   $ECHO "xprojects              none"
   $ECHO "default_jc             none"
   $ECHO "enforce_jc             false"
   $ECHO "enforce_project        false"
   $ECHO "enforce_user           auto"
   if [ "$LO" = true ]; then
      $ECHO "load_report_time       00:00:01"
   else
   ...
   if [ "$LO" = true ]; then
      $ECHO "execd_params           KEEP_ACTIVE=ERROR SIMULATE_JOBS=1"
   else
      $ECHO "execd_params           KEEP_ACTIVE=ERROR"
   fi
   $ECHO "reporting_params       accounting=true reporting=false flush_time=00:00:15 joblog=false sharelog=00:00:00"
   $ECHO "finished_jobs          0"
   ...
   $ECHO "cgroups_params         cgroup_path=none cpuset=false mount=false freezer=false freeze_pe_tasks=false killing=false forced_numa=false h_vmem_limit=false m_mem_free_hard=false m_mem_free_soft=false min_memory_limit=0"
   ...
   if [ "$LO" = true ]; then
      $ECHO "license_sensor_log_level       10"
      $ECHO "license_sensor_log_file        $QMDIR/messages_license_sensor"
      $ECHO "license_sensor_cmd_timeout     00:00:03"
   fi
}
...
GetConfiguration()
{
   ...
   while [ $done = false ]; do
      $CLEAR
      $INFOTEXT -u "\n%s cluster configuration" "$PRODUCT"
      $INFOTEXT "\nPlease give the basic configuration parameters of your %s\n" \
                "installation:\n\n   <execd_spool_dir>\n\n" "$PRODUCT"
      ...
      $INFOTEXT -u "\n%s cluster configuration (continued)" "$PRODUCT"
      $INFOTEXT -n "\n<administrator_mail>\n\n" \
...
AddDefaultJobClass()
{
   $INFOTEXT "Adding default Job Class"
   $SGE_BIN/$CONF_CMD -Ajc $SGE_ROOT_VAL/util/resources/jobclass/LO_template
...
AddDefaultUsersets()
{
      $INFOTEXT "Adding default usersets"
...
CreateSettingsFile()
{
   ...
   if [ $RECREATE_SETTINGS = "false" ]; then
      if [ -f $root_dir/$cell_dir/common/settings.sh ]; then
         ExecuteAsAdmin $RM $root_dir/$cell_dir/common/settings.sh
      fi
  
      if [ -f $root_dir/$cell_dir/common/settings.csh ]; then
         ExecuteAsAdmin $RM $root_dir/$cell_dir/common/settings.csh
      fi
   fi
   ...
   if [ "$LO" = true ]; then
      ExecuteAsAdmin util/create_lo_settings.sh $SGE_ROOT_VAL/$COMMONDIR
   else
...
InitCA()
{
   ...
   if [ "$CSP" = true -o "$WINDOWS_SUPPORT" = "true" -o \( "$SGE_ENABLE_JMX" = true -a "$SGE_JMX_SSL" = true \) ]; then
...
StartQmaster()
{
   $INFOTEXT -u "\n%s %smaster startup" "$PRODUCT" $PRODUCT_SHORT_S
   $INFOTEXT "\nStarting %smaster daemon. Please wait ..." $PRODUCT_SHORT_S
   ...
   if [ "$SGE_ENABLE_SMF" = "true" -a "$LO" = false ]; then
      ...
      if [ "$LO" = true ]; then
         $SGE_STARTUP_FILE -lomaster
      else
         if [ "$RC_FILE" = "systemctl" -a "$USE_SYSTEMD" = "true" -a "$SYSTEMD_ADD2RC" = "0" ]; then
	    $SYSTEMCTL start sgemaster.${SGE_CLUSTER_NAME}.service
	 else
	    $SGE_STARTUP_FILE -qmaster
	fi
      fi
      # TODO: qping does not support LO so far! Has to be fixed!!!
      if [ $? -ne 0 -a "$LO" = false ]; then
         $INFOTEXT "%smaster start problem" $MASTER_PREFIX_S
         $INFOTEXT -log "%smaster start problem" $MASTER_PREFIX_S
         MoveLog
         exit 1
      fi
   fi
   ...
   run=$?
   if [ $run -ne 0 ]; then
      $INFOTEXT "%smaster daemon didn't start. Please check your\n" \
                "configuration! Installation failed!" $MASTER_PREFIX_S
      $INFOTEXT -log "%smaster daemon didn't start. Please check your\n" \
                     "autoinstall configuration file! Installation failed!" $MASTER_PREFIX_S
...
AddHosts()
{
   ...
   if [ "$AUTO" = "true" ]; then
      for h in $ADMIN_HOST_LIST; do
        if [ -f $h ]; then
           ...
           for tmp in `cat $h`; do
             ...
             $SGE_BIN/$conf_cmd -ah $tmp
           done
        else
             $INFOTEXT -log "Adding ADMIN_HOST %s" $h
             $SGE_BIN/$conf_cmd -ah $h
        fi
      done
      ...
      for h in $SUBMIT_HOST_LIST; do
        if [ -f $h ]; then
           ...
           for tmp in `cat $h`; do
             ...
             $SGE_BIN/$conf_cmd -as $tmp
           done
        else
             $INFOTEXT -log "Adding SUBMIT_HOST %s" $h
             $SGE_BIN/$conf_cmd -as $h
        fi
      done  
      ...
      $INFOTEXT -u "\nAdding %s hosts" "$PRODUCT"
      # in LO we add the master host to the submit hosts list
      if [ "$LO" = true ]; then
         ACT_QMASTER=`cat $LO_ROOT/$SGE_CELL/common/act_qmaster`
         $SGE_BIN/$conf_cmd -as $ACT_QMASTER
      else
      ...
      $INFOTEXT "\nIf you want to use a shadow host, it is recommended to add this host\n" \
                ...
                "installation with <%s -ah hostname> for adding and <%s -dh hostname>\n" \
      ...
      if [ "$ret" = 0 ]; then
         ...
         $INFOTEXT "\nPlease now add the list of hosts, where you will later install your shadow\n" \
                   ...
                   "simply press <RETURN> without entering a name.\n\n" \
                   "You also may prepare a file with the hostnames of the machines where you plan\n" \
                   "to install %s. This may be convenient if you are installing \n" \
                   "%s on many hosts.\n\n" "$PRODUCT" "$PRODUCT"
   ...
   if [ "$Overwrite" = "true" -a "$SPOOLING_METHOD" = "classic" ]; then
      $INFOTEXT -u "\nSkipping creation of the default <all.q> queue, <allhosts> hostgroup and <template> job class"
   else
      $INFOTEXT -u "\nCreating the default <all.q> queue, <allhosts> hostgroup and <template> job class"
      echo
      $INFOTEXT -log "Creating the default <all.q> queue, <allhosts> hostgroup and <template> job class"
      TMPL=/tmp/hostqueue$$
      TMPL2=${TMPL}.q
      TMPL3=${TMPL}.jc
      rm -f $TMPL $TMPL2 $TMPL3
      if [ -f $TMPL -o -f $TMPL2 -o -f $TMPL3 ]; then
         $INFOTEXT "\nCan't delete template files >%s< or >%s< or >%s<" "$TMPL" "$TMPL2" "$TMPL3"
      else
		   ...
		   if [ x`$SGE_BIN/$conf_cmd -shgrpl 2>/dev/null | grep '^@allhosts$'` = x ]; then
            PrintHostGroup @allhosts > $TMPL
            Execute $SGE_BIN/$conf_cmd -Ahgrp $TMPL
			else
			...
			if [ x`$SGE_BIN/$conf_cmd -sql 2>/dev/null | grep '^all.q$'` = x ]; then
            Execute $SGE_BIN/$conf_cmd -sq > $TMPL
            if [ "$LO" = true ]; then
               Execute sed -e "/qname/s/template/all.q/" \
                           -e "/hostlist/s/NONE/@allhosts/" \
                           -e "/load_thresholds/s/np_load_avg=1.75/NONE/" \
                           -e "/pe_list/s/NONE/make,OpenMP/" $TMPL > $TMPL2
            else
               Execute sed -e "/qname/s/template/all.q/" \
                           -e "/hostlist/s/NONE/@allhosts/" \
                           -e "/pe_list/s/NONE/make,OpenMP/" $TMPL > $TMPL2
            fi
            Execute $SGE_BIN/$conf_cmd -Aq $TMPL2
			else
			...
			if [ x`$SGE_BIN/$conf_cmd -sjcl 2>/dev/null | grep '^template$'` = x ]; then
            Execute $SGE_BIN/$conf_cmd -sjc > $TMPL
            Execute $SGE_BIN/$conf_cmd -Ajc $TMPL
			else
			   $INFOTEXT "Skipping creation of <template> job class as it already exists"
				$INFOTEXT -log "Skipping creation of <template> job class as it already exists"
			fi
         rm -f $TMPL $TMPL2 $TMPL3
      fi
...
AddHostsFromTerminal()
{
   ...
   while [ $stop = false ]; do
      ...
      $INFOTEXT "\nPlease enter a blank seperated list of hosts.\n\n" \
                ...
                "entering an empty list. You will see messages from %s\n" \
...
#-------------------------------------------------------------------------
# AddAdminHost: Adds $1 as admin host. Prints error in case of failure
#
...
AddAdminHost()
{
    ...
    result=`$SGE_BIN/$conf_cmd -ah $h 2>&1 >/dev/null`
    if [ $? -ne 0 ]; then
        $INFOTEXT "Did not add $h as admin host ($result)"
        return 1
    fi
...
#-------------------------------------------------------------------------
# AddSubmitHost: Adds $1 as submit host. Prints error in case of failure
#
...
{    
    ...
    result=`$SGE_BIN/$conf_cmd -as $h 2>&1 >/dev/null`
    if [ $? -ne 0 ]; then
        $INFOTEXT "Did not add $h as submit host ($result)"
        return 1
    fi
...
GetQmasterPort()
{
   ...
   if [ "$master_port" != "" -a "$port_source" != "db" ]; then
      $INFOTEXT -u "\n%s TCP/IP communication service" "$PRODUCT"

      if [ $master_port -ge 1 -a $master_port -le $comm_port_max ]; then
         $INFOTEXT "\nUsing the environment variable\n\n" \
                   "   \$%sMASTER_PORT=%s\n\n" \
                     "as port for communication.\n\n" $MASTER_PREFIX $master_port
                      if [ "$LO" = true ]; then
                         export LO_MASTER_PORT
                      else
                         export SGE_QMASTER_PORT
                      fi
                      $INFOTEXT -log "Using %sMASTER_PORT >%s<." $MASTER_PREFIX $master_port
         ...
         $INFOTEXT "\nThe environment variable\n\n" \
                   ...
                   "Please set the environment variable \$%sMASTER_PORT and restart\n" \
                   "the installation or configure the service >%smaster<." $MASTER_PREFIX $SGE_QMASTER_PORT $comm_port_max $MASTER_PREFIX $MASTER_PREFIX_S
         $INFOTEXT -log "Your \$%s_QMASTER_PORT=%s\n\n" \
                   "has an invalid value (it must be in range 1..%s).\n\n" \
                   "Please check your configuration file and restart\n" \
                   "the installation or configure the service >%smaster<." $MASTER_PREFIX $SGE_QMASTER_PORT $comm_port_max $MASTER_PREFIX_S
      fi
   fi
   $INFOTEXT -u "\n%s TCP/IP service >%smaster<" "$PRODUCT" $MASTER_PREFIX_S
...
EnterServiceOrPortAndCheck()
{
   ...
   elif [ "$1" = "lo_master" ]; then
      service_name="lo_master"
      port_var_name="LO_MASTER_PORT"
   elif [ "$1" = "lo_execd" ]; then
      service_name="lo_execd"
      port_var_name="LO_EXECD_PORT"
   else
...
SelectedPortOutput()
{
   if [ "$1" = "sge_qmaster" ]; then
      ...
      "as port for communication.\n\n" $LO_MASTER_PORT
      export LO_MASTER_PORT
      $INFOTEXT -log "Using LO_MASTER_PORT >%s<." $LO_MASTER_PORT
   elif [ "$1" = "lo_execd" ]; then
      LO_EXECD_PORT=$INP
      $INFOTEXT "\nUsing the environment variable\n\n" \
      "   \$LO_EXECD_PORT=%s\n\n" \
      "as port for communication.\n\n" $LO_EXECD_PORT
      export LO_EXECD_PORT
      $INFOTEXT -log "Using LO_EXECD_PORT >%s<." $LO_EXECD_PORT
   else
      SGE_EXECD_PORT=$INP
      $INFOTEXT "\nUsing the environment variable\n\n" \
      "   \$SGE_EXECD_PORT=%s\n\n" \
...
EnterAndValidatePortNumber()
{
   ...
   if [ "$1" = "sge_qmaster" -o "$1" = "lo_master" ]; then
      $INFOTEXT -n "Please enter an unused port number >> "
      if [ "$LO" = true ]; then
         INP=`Enter $LO_MASTER_PORT`
         master_port=$LO_MASTER_PORT
      else
      ...
      while [ $port_ok = "false" ]; do 
         ...
         port_ok="true"
         if [ "$INP" = "$master_port" -a $1 = "sge_execd" -a "$LO" = false -o \
             "$INP" = "$master_port" -a $1 = "lo_execd" -a "$LO" = true ]; then
            $INFOTEXT "\nPlease use any other port number!"
            $INFOTEXT "Port number %s is already used by %smaster\n" $master_port $MASTER_PREFIX_S
            port_ok="false"
            if [ $AUTO = "true" ]; then
               $INFOTEXT -log "Please use any other port number!"
               $INFOTEXT -log "Port number %s is already used by %smaster" $master_port $MASTER_PREFIX_S
...
GetExecdPort()
{
    ...
    if [ "$execd_port" != "" -a "$port_source" != "db" ]; then
      $INFOTEXT -u "\n%s TCP/IP communication service" "$PRODUCT"

      if [ $execd_port -ge 1 -a $execd_port -le $comm_port_max ]; then
         $INFOTEXT "\nUsing the environment variable\n\n" \
                   "   \$%s_EXECD_PORT=%s\n\n" \
                     "as port for communication.\n\n" $PRODUCT_SHORT $execd_port
                      export SGE_EXECD_PORT
                      $INFOTEXT -log "Using %s_EXECD_PORT >%s<." $PRODUCT_SHORT $execd_port
         if [ "$collision_flag" = "services_only" -o "$collision_flag" = "services_env" ]; then
            $INFOTEXT "This overrides the preset TCP/IP service >%s_execd<.\n" $PRODUCT_SHORT_S
         fi
         $INFOTEXT -auto $AUTO -ask "y" "n" -def "n" -n "Do you want to change the port number? (y/n) [n] >> "
         if [ "$?" = 0 ]; then
         ...
         $INFOTEXT "\nThe environment variable\n\n" \
                   "   \$%s_EXECD_PORT=%s\n\n" \
                   "has an invalid value (it must be in range 1..%s).\n\n" \
                   "Please set the environment variable \$%s_EXECD_PORT and restart\n" \
                   "the installation or configure the service >%s_execd<." $PRODUCT_SHORT $execd_port $comm_port_max $PRODUCT_SHORT $PRODUCT_SHORT_S
         $INFOTEXT -log "Your \$%s_EXECD_PORT=%s\n\n" \
                   "has an invalid value (it must be in range 1..%s).\n\n" \
                   "Please check your configuration file and restart\n" \
                   "the installation or configure the service >%s_execd<." $PRODUCT_SHORT $execd_port $comm_port_max $PRODUCT_SHORT_S
      fi
   fi
   ...
   if [ "$port_source" = "env" ]; then
         ...
         $INFOTEXT "Make sure to use a different port number for the execution host\n" \
                   "as on the %smaster machine\n" $MASTER_PREFIX_S
         if [ `$SGE_UTILBIN/getservbyname $SGE_QMASTER_SRV 2>/dev/null | wc -w` = 0 -a "$master_port" != "" ]; then
            $INFOTEXT "The %smaster port %sMASTER_PORT = %s\n" $MASTER_PREFIX_S $MASTER_PREFIX $master_port
         elif [ `$SGE_UTILBIN/getservbyname $SGE_QMASTER_SRV 2>/dev/null | wc -w` != 0 -a "$master_port" = "" ]; then
            $INFOTEXT "%smaster service set to port %s\n" $MASTER_PREFIX_S `$SGE_UTILBIN/getservbyname $SGE_QMASTER_SRV | cut -d" " -f2`
         else 
            $INFOTEXT "The %smaster port %sMASTER_PORT = %s" $MASTER_PREFIX_S $MASTER_PREFIX $master_port
            $INFOTEXT "%smaster service set to port %s\n" $MASTER_PREFIX_S `$SGE_UTILBIN/getservbyname $SGE_QMASTER_SRV | cut -d" " -f2`
         fi 
         $INFOTEXT -wait -auto $AUTO -n "Hit <RETURN> to continue >> "
...
PortCollision()
{
   ...
   case "$collision_flag" in
      ...
      env_only)
         ...
         elif [ "$service" = "lo_master" ]; then
            $INFOTEXT "   LO_MASTER_PORT = %s" $LO_MASTER_PORT
         elif [ "$service" = "lo_execd" ]; then
            $INFOTEXT "   LO_EXECD_PORT = %s" $LO_EXECD_PORT
         else
      ...
      services_only)
         ...
         if [ "$service" = "sge_qmaster" ]; then
            ...
            $INFOTEXT "   lo_master service set to port %s" `$SGE_UTILBIN/getservbyname $service | cut -d" " -f2` 
         elif [ "$service" = "lo_execd" ]; then
            $INFOTEXT "   lo_execd service set to port %s" `$SGE_UTILBIN/getservbyname $service | cut -d" " -f2` 
         else
      ...
      services_env)
         ...
         if [ "$service" = "sge_qmaster" ]; then
            ...
            $INFOTEXT "   LO_MASTER_PORT = %s" $LO_MASTER_PORT
            $INFOTEXT "   lo_master service set to port %s" `$SGE_UTILBIN/getservbyname $service | cut -d" " -f2` 
            $INFOTEXT "\n   Currently LO_MASTER_PORT = %s is active!" $LO_MASTER_PORT 
         elif [ "$service" = "lo_execd" ]; then
            $INFOTEXT "   LO_EXECD_PORT = %s" $LO_EXECD_PORT
            $INFOTEXT "   lo_execd service set to port %s" `$SGE_UTILBIN/getservbyname $service | cut -d" " -f2` 
            $INFOTEXT "\n   Currently LO_EXECD_PORT = %s is active!" $LO_EXECD_PORT 
         else
            $INFOTEXT "   SGE_EXECD_PORT = %s" $SGE_EXECD_PORT
            $INFOTEXT "   sge_execd service set to port %s" `$SGE_UTILBIN/getservbyname $service | cut -d" " -f2` 
...
PortSourceSelect()
{
   ...
   $INFOTEXT -n "How do you want to configure the %s communication ports?\n\n" "$PRODUCT"
